...
\begin{example}
  \label{ex:running 2}
  The skew-symmetric matrix $\Omega=\left[\begin{array}{cc} 0 & 1\\ -1 & 0\end{array}\right]$ from Example~\ref{ex:running 1} is compatible (up to a sign) with any $2\times 2$ skew-symmetrizable matrix $\tilde B$ where $D$ is determined directly by the off-diagonal entries of $\tilde B$.
\end{example}
...
\begin{example}
  \label{ex:running 3}
  As in Example~\ref{ex:running 2}, we take 
  ...
  Taking $\bfr=(1,3)$, we get $Z_1(u)=1+u$ and $Z_2(u)=1+z_1u+z_2u^2+u^3$, where we write $z_j$ instead of $z_{2,j}$ for $j=1,2$.
  ...
  \end{align*}
  For example, the cluster variables $x_{1;t_3}$ and $x_{2;t_3}$ are given by
  \begin{align*}
    x_{1;t_3}
...
\begin{example}
  Continuing from Example~\ref{ex:running 3}, we see from Lemma~\ref{le:dual vectors} that $C_t^\vee=C_t^T$ and $G_t^\vee=G_t^T$ for $t\in\TT_n$, where $(-)^T$ denotes the transpose of a matrix.
