...
For a smooth Poisson manifold $(M, \pi)$ with a Poisson spray $X \in \cT_{T^*M}$, a neighbourhood $U$ of the zero section of $T^*X$ is a local symplectic groupoid over $(M, \pi)$ with the following structures:
	\begin{enumerate}
		\item the source map $\alpha = \tau: U \to M$ is the bundle projection ;
...
\begin{remark}
	By a local symplectic groupoid $(\cG, \omega) \to (M, \pi)$, we mean that the multiplication $m: \cG {_\beta \times_\alpha} \cG \to \cG$ may not be defined on all of its domain. In this case, the time-$1$-flow of the Poisson spray $X$ may be outside $U$.
...
Let $L$ be either $\RR^m$ or $\CC^m$ and $\{(x_1, \ldots, x_n\}$ be coordinates on $L$, and let $\Omega_{ij}, 1 \leq i,j \leq m$ be a skew-symmetric matrix. For the log-canonical Poisson structure $\{x_i, x_j\} = \Omega_{ij} x_i x_j$, or equivalently $\pi = \sum_{i > j} \Omega_{ij} x_ix_j\frac{\partial}{\partial x_i} \wedge \frac{\partial}{\partial x_j}$, the vector field $X \in \cT_{T^*L}$:
...
there is a source-simply-connected symplectic groupoid $(\cG_L, \omega) \rra (L, \pi)$ with the following structures:
	\begin{enumerate}
		...
		\item the source map is the bundle projection
			$$
				\alpha: \cT^*L \to L, \qquad (p_1, \ldots, p_m, x_1, \ldots, x_m) \mapsto (x_1, \ldots, x_m);
			$$
		\item the target map is
			$$
				\begin{aligned}
		...
		\item the multiplication map is
			$$
				\begin{aligned}
				m_t: & \cT^*L {_\beta \times_\alpha} \cT^*L \to \cT^*L, \\
...
\begin{proof}
	The flow $\varphi_X^t: T^*L \to T^*L$ of the Poisson spray $X$ as in \eqref{eq: PoisSp} is as follows:
...
There is another interesting symplectic groupoid of the log canonical Poisson space $(L, \pi)$, the symplectic double in \cite{MR2470108}. We denote this symplectic groupoid by $\cD_L \rra L$.

{\color{red} Introduce the symplectic double from \cite{MR2470108} in local charts.}

The symplectic groupoid $\cG_L \rra L$, being source-simply-connected, is the covering of $\cD_L \rra L$ such that each source fiber of $\cG_L \rra L$ is the universal cover of the corresponding source fiber of $\cD_L \rra L$.
