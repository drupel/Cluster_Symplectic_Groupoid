...
We fix such a matrix $D$
Such an $m\times n$ matrix $\tilde B$ with skew-symmetrizable principal submatrix $B$ is called an \emph{exchange matrix}.
Fix a skew-symmetrizing matrix $D$.
An $m\times m$ matrix $\Omega=(\Omega_{ij})$ is \emph{compatible} with $(\tilde B,D)$ if $\tilde B^T\Omega=[D\ 0]$.
An $m\times r$ matrix $\Theta=(\Theta_{ij})$ is called a \emph{grading} of $\tilde B$ if $\tilde B^T\Theta=0$.
...
For $1\le k\le n$, let $E_k$ be the $m\times m$ matrix with entries
\[E_{ij}=\begin{cases}\delta_{ij} & \text{if $j\ne k$;}\\ -1 & \text{if $i=j=k$;}\\ [b_{ik}]_+ & \text{if $i\ne j=k$.}\end{cases}\]

Let $\cF$ be an extension field of $\QQ$ of transcendence degree $m$.   
A \emph{graded seed} in $\cF$ is a triple $\Sigma=(\bfx,\tilde B,\Theta)$, where $\bfx=(x_1,\ldots,x_m)$ is a transendence basis of $\cF$ over $\QQ$ called the \emph{cluster} with entries called \emph{cluster variables}, $\tilde B$ is an $m\times n$ exchange matrix, and $\Theta$ is a grading of $\tilde B$.
For $1\le k\le n$, define the \emph{mutation of $\Sigma$ in direction $k$} by $\mu_k\Sigma=(\mu_k\bfx,\mu_k\tilde B,\mu_k\Theta)$, where $\mu_k\Theta=E_k^T\Theta$ and $\mu_k\bfx=(x'_1,\ldots,x'_m)$ is given by the \emph{exchange relations}
\begin{equation}
...
An easy calculation also shows that $\mu_k\Theta$ is again a grading of $\mu_k\tilde B$ \cite{berenstein-zelevinsky, grabowski}.
A graded seed $\Sigma'$ is \emph{mutation equivalent} to $\Sigma$ if there exists a sequence of mutations which transforms $\Sigma$ into $\Sigma'$, in this case we write $\Sigma'\sim\Sigma$.
\begin{definition}
  Let $\Sigma$ be a graded seed in $\cF$.  The \emph{cluster algebra} $\cA(\Sigma)$ is the $\ZZ$-subalgebra of $\cF$ generated by all cluster variables from graded seeds $\Sigma'$ mutation equivalent to $\Sigma$.
\end{definition}
...
\end{theorem}
The grading $\Theta$ of the seed $\Sigma$ plays the following role.
\begin{theorem}
  \cite{gekhtman-shapiro-vainshtein, berenstein-zelevinsky, grabowski}
  Let $\Sigma=(\bfx,\tilde B,\Theta)$ be a graded seed.
  The $\ZZ^r$-grading on $\bfx=(x_1,\ldots,x_m)$, where $\deg_\Theta(x_i)$ is given by the $i$-th row of $\Theta$, induces a $\ZZ^r$-grading on the cluster algebra $\cA(\Sigma)$.
  Put another way, under the embedding of $\cA(\Sigma)$ into $\ZZ[x_1^{\pm1},\ldots,x_m^{\pm1}]$ all cluster variables are homogeneous with respect to this $\ZZ^r$-grading.
\end{theorem}

\begin{example}
  Given an $n\times n$ skew-symmetrizable matrix $B$, let $\tilde B_{prin}$ denote the $2n\times n$ matrix with principal submatrix $B$ and lower $n\times n$ submatrix given by $I_n$.
  In this case a graded seed $\Sigma=(\bfx_{prin},\tilde B_{prin},\Theta_{prin})$ is said to have \emph{principal coefficients} and admits the $2n\times n$ grading matrix $\Theta_{prin}$ with first $n$ rows given by $I_n$ and last $n$ rows given by $-B^T$.
  Then the grading vector $\deg_{\Theta_{prin}}(x)$ of a cluster variable $x\in\cA(\Sigma)$ is known as the \emph{$g$-vector} of $x$ \cite{fomin-zelevinsky4} and will be denoted $g_B(x)$.
\end{example}
...
Observe that $\Sigma=(\bfx,\tilde B,\Theta)$ is a graded seed in the field of rational functions on this cluster chart and thus we denote it by $L_\Sigma$.
...
By composing these \emph{elementary transition maps} for neighboring graded seeds we get a birational transformation between $\varphi_{\Sigma,\Sigma'}:L_\Sigma\to L_{\Sigma'}$ for any seeds $\Sigma\sim\Sigma'$.
...
\begin{theorem}
  ...
  Given a graded seed $\Sigma=(\bfx,\tilde B,\Theta)$, the grading $\deg_\Theta$ on $\FF[x_1^{\pm1},\ldots,x_m^{\pm1}]$ induces an action of the torus $(\FF^*)^r$ on $L_\Sigma$.
  These local toric actions glue to give a global toric action on the cluster manifold $M(\Sigma)$.
\end{theorem}
%Note that we have $M(\Sigma)=M(\Sigma')$ whenever $\Sigma\sim\Sigma'$.
...
\end{definition}
\begin{remark}
  Suppose the cluster variables of a graded seed $\Sigma=(\bfx,\tilde B,\Theta)$ are log-canonical with respect to a Poisson bracket $\{\cdot,\cdot\}:\cA(\Sigma)\times\cA(\Sigma)\to\cA(\Sigma)$ with coefficient matrix $\Omega$.
...
\begin{itemize}
  \item first, we show that the action groupoids $(\FF^*)^r\times L_{\Sigma'}\rightrightarrows L_{\Sigma'}$ over each cluster chart admit a gluing which lifts the cluster mutation;
  \item second, we define maps $T^*L_{\Sigma'}\to(\FF^*)^r\times L_{\Sigma'}$ along which we pullback the groupoid structure to obtain symplectic groupoids integrating a compatible Poisson structure on $L_{\Sigma'}$;
...
To begin, we identify $\ZZ^r$ with the character lattice $\Hom((\FF^*)^r,\FF^*)$ and write $\Theta_i$ for the character corresponding to the $i$-th row of $\Theta$.
There is a groupoid structure $(\FF^*)^r\times L_\Sigma\rightrightarrows L_\Sigma$ with source map $\alpha$ being the natural projection and target map given by the twisted Hadamard product
\[\beta(\bfs,\bfx)=\bfs^\Theta\circ\bfx,\quad \bfs^\Theta=(\bfs^{\Theta_1},\ldots,\bfs^{\Theta_m}),\]
i.e.\ given by the action of $(\FF^*)^r$ on $L_\Sigma$ determined by $\Theta$.
...
Define a map $\rho_\Sigma:G_\Sigma\to(\FF^*)^r\times L_\Sigma$ by $\rho_\Sigma(\bfx,\bfp)=(\bfs(\bfx,\bfp),\bfx)$, with $s_i(\bfx,\bfp)=e^{\sum_j\Omega_{ij}x_jp_j}$.
...
\begin{theorem}
  ...
  The groupoid structure on $(\FF^*)^r\times L_\Sigma$ pulls back to a groupoid structure on the manifold $G_\Sigma$ with source map the natural projection, target map $\beta\circ\rho_\Sigma$, multiplication given by
...
\begin{equation}
  ...
  \mu_{\Sigma,\mu_k\Sigma}(\bfx,\bfp)=(\bfx'(\bfx),\bfp'(\bfx,\bfp)),\quad \bfp'(\bfx,\bfp)=(p'_1(\bfx,\bfp),\ldots,p'_m(\bfx,\bfp)),\quad p'_\ell(\bfx,\bfp)=\frac{\sum\limits_{i\ne k} b'_{i\ell}\sum\limits_j \Omega_{ij}x_jp_j-b_{kl}\ln\left(\frac{x'_k(\bfs\circ\bfx)}{x'_k(\bfx)}\right)}{d_\ell x'_\ell(\bfx)}
\end{equation}
