...
This is the first of a series of papers whose aim is to implement the Weinstein's program of geometric quantization for Poisson manifolds \cite{MR1104934} in the case of cluster Poisson structures.
...
The cluster $\cA$-varieties are often endowed with a class of compatible Poisson structures in the sense of Gekhtman, Shapiro, and Vainshtein \cite{GSV10}. The cluster $\cX$-varieties were introduced by Fock and Goncharov \cite{FG09a} in their study of higher Teichmuller space.
...
In the case of cluster algebras, the quantum $\cA$-varieties \cite{berenstein-zelevinsky} and the quantum $\cX$-varieties \cite{FG09c} are both concrete examples of quantization of Poisson manifolds. In fact, the q-quantization of log-canonical variables is simply the exponential version of the Weyl quantization of canonical variables.
Geometric quantization for symplectic manifolds takes one step further by constructing a Hilbert space, but this requires that the cohomology class of the symplectic 2-form is integral (see e.g.\ \cite{MR1806388}).
...
In Section~\ref{sec: local}, we construct three groupoids $\cG$, $\cB$ and $\cD$ for a log-canonical Poisson structure $\pi$ on a vector space $L$. The groupoid $\cG \rra L$ is the source-simply-connected symplectic groupoid when the underlying field is $\CC$. The construction of $\cG \rra L$ utilizes the construction of local symplectic groupoids by Poisson sprays \cite{MR2900786, CMS17}. For a log-canonical Poisson structure, there is a natural choice of a Poisson spray such that the spray construction yields $\cG$. The source and the target maps of $\cG \rra L$ are transcendental. 

The groupoid $\cD \rra L$ is a source-connected Poisson groupoid. The restriction of $\cD \rra L$ over an octant $L_+$ of $L$, which we denote by $\cD_+ \rra L_+$ is symplectic, is essentially the symplectic double in \cite{FG09c}, albeit with slightly different notations to accommodate the case of the non-square exchange matrices.

The groupoid $\cB \rra L$ is a source-connected symplectic groupoid, obtained from $\cD \rra L$ by a blow-up construction. Comparing to $\cG \rra L$, the advantage of $\cB \rra L$ is that the source and the target maps of $\cG \rra L$ are rational.
...
In Section~\ref{sec:cluster}, we give the explicit formulae for the cluster groupoid mutation $\mu: \cG \to \cG'$, $\mu: \cB \to \cB'$ and $\mu: \cD \to \cD'$. The first step is to decompose a generalized cluster mutation $\mu: L \to L'$ into two maps $\varphi^1$ and $\tau$. The first map $\varphi^1$ is the time-$1$-flow of a Hamiltonian vector field defined using the Euler dilogarithm function; the second map $\tau$ is a natural transformation of the log-canonical coordinates \cite{FG09a, MR3691969}. We then lift $\varphi^1$ and $\tau$ to groupoid maps, and the composition of the lifted groupoid maps is the cluster groupoid mutation. We also lift the cluster ensemble map $\rho: L_\cA \to L_\cX$ to groupoid maps. When the underlying field is $\RR$, the Poisson map $\rho$ can be lifed to all three groupoids $\cG$, $\cD$ and $\cB$. When the underlying field is $\CC$, the Poisson map $\rho$ can only lifed to the source-simply-connected symplectic groupoid $\cG$.
\bigskip
...
%	\]
	\item For a vector bundle $E$ over $M$, we denote the space of sections by $\Gamma(M, E)$. In particular, we denote the space of vector fields by $\cT_M = \Gamma(M, TM)$, but to follow the conventional notation, we denote the space of differential $1$-forms by $\Omega^1(M) = \Gamma(M, T^*M)$.
...
\begin{theorem} 
  ...
  \begin{enumerate}
    ...
    \item the target map is
      \[\beta: \cB \to L, \qquad (\bfs, \bfx) \mapsto \left(x_1 \prod_{i=1}^m (u_ix_i+1)^{\Omega_{i1}}, \ldots, x_m \prod_{i=1}^m (u_ix_i+1)^{\Omega_{im}}\right);\]
...
\begin{corollary}
  ...
  \begin{align*}
    (\varphi_{\cX,\bfr}^t)^*(q_\ell y_\ell)&=q_\ell y_\ell-t\frac{\delta_{k\ell}}{d_k}\log\left(\frac{F_k\left(y_k^\varepsilon e^{\varepsilon\sum_{\ell'=1}^n d_{\ell'} B_{\ell' k}q_{\ell'} y_{\ell'}}\right)}{F_k(y_k^\varepsilon)}\right),\\
    (\varphi_{\cX,\bfr}^t)^*(v_\ell y_\ell)&=\left(\frac{F_k\left(y_k^\varepsilon \prod_{\ell'=1}^n (v_{\ell'} y_{\ell'} + 1)^{\varepsilon d_{\ell'} B_{\ell' k}}\right)}{F_k(y_k^\varepsilon)}\right)^{-t\frac{\delta_{k\ell}}{d_k}}(v_\ell y_\ell+1)-1,\\
    (\varphi_{\cX,\bfr}^t)^*(t_\ell)&=\left(\frac{F_k\left(y_k^\varepsilon \prod_{\ell'=1}^n t_{\ell'}^{\varepsilon d_{\ell'} B_{\ell' k}}\right)}{F_k(y_k^\varepsilon)}\right)^{-t\delta_{k\ell}/d_k}t_\ell;
  \end{align*}
  ...
  \begin{align*}
    (\varphi_{\cA,\bfr}^t)^*(p_jx_j)&=p_jx_j-t\frac{B_{jk}}{d_k}\log\left(\frac{F_k\left(\hat y_k^\varepsilon e^{-\varepsilon d_kp_kx_k}\right)}{F_k(\hat y_k^\varepsilon)}\right), \\
    (\varphi_{\cA,\bfr}^t)^*(u_jx_j)&=\left(\frac{F_k\left(\hat y_k^\varepsilon (u_kx_k+1)^{-\varepsilon d_k}\right)}{F_k(\hat y_k^\varepsilon)}\right)^{-t\frac{B_{jk}}{d_k}}(u_jx_j+1)-1, \\
    (\varphi_{\cA,\bfr}^t)^*(s_j)&=\left(\frac{F_k\left(\hat y_k^\varepsilon s_k^{-\varepsilon d_k}\right)}{F_k(\hat y_k^\varepsilon)}\right)^{-tB_{jk}/d_k}s_j.
  \end{align*}
