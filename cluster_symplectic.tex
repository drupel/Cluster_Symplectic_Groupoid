...
\begin{abstract}
  ...
  The resulting groupoid structures are also valid in the complex case.
...
Weinstein introduced symplectic groupoids as a tool for quantizing Poisson manifolds in the spirit of the geometric quantization of symplectic manifolds.
In this construction, the prequantum line bundle typically associated to a symplectic manifold (which does not exist for Poisson manifolds) is replaced by a prequantum line bundle on the symplectic groupoid of the Poisson manifold. 
Then geometric quantization is carried out in this setting and pushed down to the Poisson manifold using the source or target map of the symplectic groupoid.
...
In this paper, we construct the symplectic groupoids for both the cluster $\cX$-varieties and the cluster $\cA$-varieties which admit a Poisson structure compatible with the cluster structure. 
...
A cluster $\cA$-variety is naturally endowed with a presymplectic form which can often be upgraded to a compatible Poisson structure in the sense of Gekhtman, Shapiro, and Vainshtein \cite{GSV10}.
...
We provide lifts of this cluster ensemble map to comorphisms between analogous groupoids over the $\cA$- and $\cX$-varieties.
...
Geometric quantization for symplectic manifolds takes one step further by constructing a Hilbert space on which the quantized algebra acts, however note that this requires the cohomology class of the symplectic 2-form to be integral (see e.g.\ \cite{BW97}).
...
Weinstein's motivation was to find a geometric quantization schema for Poisson manifolds \cite{Wei91}, which has since only been successfully implemented for a handful of examples \cite{Tan06, Haw08, BCST12}.
...
We begin in Section~\ref{sec:Poisson generalities} with a general discussion of Poisson manifolds and their associated algebroids and groupoids.
...
The groupoid $\cG \rra L$ is the source-simply-connected symplectic groupoid which then provides a covering for all other groupoids over $L$.
The definition of $\cG \rra L$ utilizes the construction of local symplectic groupoids via Poisson sprays \cite{CM11, CMS17}.
...
Over an orthant $L^\times$ of $L$ the groupoid $\cD \rra L$ is actually symplectic and we denote its restriction by $\cD^\times \rra L^\times$.
Our construction gives a covering of the symplectic groupoid considered by Fock and Goncharov in the case of $\cX$-varieties.
...
Observe that both $\cB \rra L$ and $\cD \rra L$ naturally receive maps from the source-simply-connected symplectic groupoid $\cG \rra L$ (c.f.\ \eqref{eq:LGpdCD}).

Analogous to the construction of cluster varieties by gluing cluster charts along birational isomorphisms, the groupoids of the cluster $\cA$-variety and cluster $\cX$-variety are not constructed globally but rather are glued from groupoid charts over each cluster chart.
...
The paper concludes by showing that periodicities in the mutations of cluster charts lift to periodicities of mutations of groupoid charts.
This is accomplished by extending the $\bfc$-vector, $\bfg$-vector, $F$-polynomial construction of cluster variables to the groupoid coordinate charts.
\bigskip
...
\begin{itemize}
	...
	\item We denote vectors by boldface, e.g. $\bfx = (x_1, \ldots, x_m)$.
        \item The \emph{Hadamard product} of two vectors $\bfx$ and $\bfy$ is given by the \emph{vector} $\bfx \circ \bfy = (x_1y_1, \ldots, x_my_m)$.
...
%	\]
	...
	\item For a vector bundle $E$ over $M$, we denote the space of sections by $\Gamma(M, E)$.
          In particular, we denote the space of vector fields by $\cT_M := \Gamma(M, TM)$ and denote the space of multi-vector fields by $\cT^k_M := \Gamma(M, \wedge^k TM)$.
...
\begin{definition} \label{def:groupoid}
  ...
  \begin{enumerate}
    ...
    \item an involutive inversion map $\iota: \cG \to \cG, \enskip g \mapsto g^{-1}$;
  \end{enumerate}
...
\end{equation}
Since $A$ is a subbundle of $T\cG|_{\mathtt{1}_M}$, a section of $A$ can be thought of as a section of $T\cG$ supported on $\mathtt{1}_M$, which can be left translated along the source fibers of $\cG\rightrightarrows M$ to yield a left-invariant vector field on $\cG$.
Similar to Lie groups, the left-invariant vector fields of a Lie groupoid are closed under the Lie bracket of vector fields, and so we have a Lie bracket on $\Gamma(M, A)$.
On the other hand, the anchor map is the restriction of the target map $\beta_*: T\cG \to TM$ to $A$.
If $A \cong \Lie\cG$, we say the Lie groupoid $\cG$ \emph{integrates} the Lie algebroid $A$.
...
\begin{definition}
  ...
  That is, $\mathrm{pr}_1^*(\omega)+\mathrm{pr}_2^*(\omega) = m^*(\omega)$ or equivalently the graph of the multiplication map $\Gamma_m := \{(g, h, gh) \in \cG \times \cG \times \cG\}$ is Lagrangian with respect to the symplectic structure $\omega \oplus \omega \oplus -\omega$ on $\cG\times\cG\times\cG$.
\end{definition}
...
The Lie algebroid of a symplectic groupoid $\cG \rra M$ is the cotangent bundle $T^*M$ \cite{Wei87} with the anchor map
...
\begin{example}
  \label{ex:running 1}
  We initiate here a running example with $m=2$ and $\{x_1,x_2\}=x_1x_2$, i.e.\ $\pi=x_1x_2\frac{\partial}{\partial x_1} \wedge \frac{\partial}{\partial x_2}$.
  In this case, we have $\Omega=\left[\begin{array}{cc} 0 & 1\\ -1 & 0\end{array}\right]$.
\end{example}

Using the results in \cite{CM11, CMS17}, we construct the source-simply-connected symplectic groupoid of a log-canonical Poisson structure by choosing an appropriate Poisson spray. 
\begin{definition} 
  \cite{CM11}
...
\end{theorem}
\begin{proof}
  When the underlying field is $\RR$, it is straightforward to check that the Poisson spray \eqref{eq: PoisSp} induces the given groupoid structures. To find the symplectic structure $\omega_\cG$, we have
  \begin{align*}
...
\begin{example}
  Continuing with Example~\ref{ex:running 1}, we obtain the Poisson spray on $L$:
  \[X=x_1p_1x_2\frac{\partial}{\partial x_2}-x_2p_2x_1\frac{\partial}{\partial x_1}-x_1p_1p_2\frac{\partial}{\partial p_2}+x_2p_2p_1\frac{\partial}{\partial p_1}.\]
  This gives rise to the source-simply-connected symplectic groupoid $\cG$ with target map $\beta:\cG\to L$ given by $\beta(x_1,x_2,p_1,p_2)=(e^{-x_2p_2}x_1,e^{x_1p_1}x_2)$ and multiplicative Poisson bi-vector
  \[\sigma_\cG=-\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial p_1}-\frac{\partial}{\partial x_2}\wedge \frac{\partial}{\partial p_2}-x_1p_2\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial p_2}+x_2p_1\frac{\partial}{\partial x_2}\wedge \frac{\partial}{\partial p_1}+p_1p_2\frac{\partial}{\partial p_1}\wedge \frac{\partial}{\partial p_2}+x_1x_2\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial x_2}.\]
\end{example}
...
\begin{example}
  For $L$ as in Example~\ref{ex:running 1}, we obtain the source-connected Poisson groupoid $\cD$ with target map $\beta:\cD\to L$ given by $\beta(x_1,x_2,s_1,s_2)=(x_1s_2^{-1},x_2s_1)$ and multiplicative Poisson bi-vector
  \[\sigma_\cD=-x_1s_1\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial s_1}-x_2s_2\frac{\partial}{\partial x_2}\wedge \frac{\partial}{\partial s_2}+x_1x_2\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial x_2}.\]
\end{example}

It may seem strange that the Poisson groupoid $\cD \rra L$ is called the \emph{symplectic} double since the Poisson structure drops rank along the coordinate axes.
...
\begin{definition}
  Let $E$ be a vector bundle over $M$ and let $F$ be a proper subbundle of $E|_N$ for some hypersurface $N\subset M$.
  The \emph{elementary (lower) modification} of $E$ along $F$, denoted by $[E\!:\!F]$, is the vector bundle with the sheaf of sections
...
Let $L_k$ be the codimension-1 subspace in $L$ defined by $x_k = 0$.
...
\begin{example}
  For $L$ as in Example~\ref{ex:running 1}, we obtain the source-connected Poisson groupoid $\cB$ with target map $\beta:\cB\to L$ given by $\beta(x_1,x_2,u_1,u_2)=\big(x_1(x_2u_2+1)^{-1},x_2(x_1u_1+1)\big)$ and multiplicative Poisson bi-vector
  \begin{align*}
    \sigma_\cB&=-(x_1u_1+1)\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial u_1}-(x_2u_2+1)\frac{\partial}{\partial x_2}\wedge \frac{\partial}{\partial u_2}-x_1u_2\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial u_2}\\
    &\quad+x_2u_1\frac{\partial}{\partial x_2}\wedge \frac{\partial}{\partial u_1}+u_1u_2\frac{\partial}{\partial u_1}\wedge \frac{\partial}{\partial u_2}+x_1x_2\frac{\partial}{\partial x_1}\wedge \frac{\partial}{\partial x_2}.
  \end{align*}
\end{example}

\begin{remark}
  Recall that the infinitesimal object of a Poisson groupoid is a Lie bialgebroid \cite{MX94}.
  The Lie bialgebroid of the Poisson groupoid $\cD \rra L$ is isomorphic to $\big(A_\cD, TL(-\log D)\big)$, where $TL(-\log D)$ is the log-tangent bundle with respect to the normal crossing divisor $D = L_1 + L_2 + \ldots + L_m$ \cite{GL14}.
  That is,
...
Note that the existence of a matrix $\Omega$ compatible with $\tilde B$ implies $\tilde B$ has rank $n$, in other words the columns of $\tilde B$ are linearly independent.

\begin{example}
  \label{ex:running 2}
  The skew-symmetric matrix $\Omega=\left[\begin{array}{cc} 0 & 1\\ -1 & 0\end{array}\right]$ from Example~\ref{ex:running 1} is $D$-compatible with a skew-symmetrizable matrix $\tilde B=\left[\begin{array}{cc} 0 & c\\ -b & 0\end{array}\right]$ with $b,c>0$ for $D=\diag(b,c)=\left[\begin{array}{cc} b & 0\\ 0 & c\end{array}\right]$.
\end{example}
...
\end{equation}
There is a natural \emph{star-involution} $z_{\ell,j}\mapsto z^*_{\ell,j}:=z_{\ell,r_\ell-j}$ on $L_\bfr$ and we write $Z^*_\ell(u)$ for the image of $Z_\ell(u)$ under this involution, more directly $Z^*_\ell(u)=u^{r_\ell} Z_\ell(u^{-1})$ is the polynomial with reversed coefficients.
...
\begin{remark}
  ...
  When $r_\ell=1$ for all $\ell$, the construction above gives rise to the ordinary cluster varieties $\cX$ and $\cA$ \cite{FZ02,FG09a} (c.f.\ Remark~\ref{rem:cluster case}).
\end{remark}

For both the cluster $\cA$-varieties \cite{GSV10} and cluster $\cX$-varieties \cite{FG09c}, a Poisson structure is \emph{compatible} with mutations if the coordinates of each cluster chart are log-canonical (c.f.\ equation \eqref{eq:brackets}).
\begin{theorem}
  \cite{GSV10}
...
\begin{example}
  \label{ex:running 3}
  In Example~\ref{ex:running 2}, we take $b=c=1$ so that
  \[\Omega=\left[\begin{array}{cc} 0 & 1\\ -1 & 0\end{array}\right]\qquad\text{and}\qquad \tilde B=\left[\begin{array}{cc} 0 & 1\\ -1 & 0\end{array}\right],\]
  which are $D$-compatible for $D=\diag(1,1)$.
  In this small dimension, mutation of $D$-compatible pairs as in \eqref{eq:matrix mutation} is just given by negation of matrices.

  In the cluster case $\bfr=(1,1)$, we have $Z_1(u)=1+u=Z_2(u)$.
  This example exhibits a 5-fold periodicity in the sense of Definition~\ref{def:periodicity} with $\sigma$ the non-identity permutation of the set $\{1,2\}$.
  In particular, to compute all the $x_{j;t}$ and $y_{\ell;t}$ it suffices to consider a portion of the exchange tree $\TT_n$ as below:
  \[t_0 \stackrel{1}{\longleftrightarrow} t_1 \stackrel{2}{\longleftrightarrow} t_2 \stackrel{1}{\longleftrightarrow} t_3 \stackrel{2}{\longleftrightarrow} t_4 \stackrel{1}{\longleftrightarrow} t_5,\]
  here $t_5$ can be identified with $t_0$ via $\sigma$ by periodicity.
  The $C$-matrices and $G$-matrices can be computed as:
  \begin{align*}
    C_{t_0}=\left[\begin{array}{cc} 1 & 0\\ 0 & 1\end{array}\right] &&
    C_{t_1}=\left[\begin{array}{cc} -1 & 1\\ 0 & 1\end{array}\right] &&
    C_{t_2}=\left[\begin{array}{cc} 0 & -1\\ 1 & -1\end{array}\right] &&
    C_{t_3}=\left[\begin{array}{cc} 0 & -1\\ -1 & 0\end{array}\right] &&
    C_{t_4}=\left[\begin{array}{cc} 0 & 1\\ -1 & 0\end{array}\right] \\
    G_{t_0}=\left[\begin{array}{cc} 1 & 0\\ 0 & 1\end{array}\right] &&
    G_{t_1}=\left[\begin{array}{cc} -1 & 0\\ 1 & 1\end{array}\right] &&
    G_{t_2}=\left[\begin{array}{cc} -1 & -1\\ 1 & 0\end{array}\right] &&
    G_{t_3}=\left[\begin{array}{cc} 0 & -1\\ -1 & 0\end{array}\right] &&
    G_{t_4}=\left[\begin{array}{cc} 0 & 1\\ -1 & 0\end{array}\right] 
  \end{align*}
  The cluster variables can then be computed from the separation of additions formulas \eqref{eq:separation of additions 1} and \eqref{eq:separation of additions 2} using the $F$-polynomials below (the permutation $\sigma$ can be observed in the first equality):
  \[ F_{\bfr,2;t_4}=F_{\bfr,1;t_0}=1 \hspace{1in} F_{\bfr,2;t_0}=F_{\bfr,2;t_1}=1 \]
  \[ F_{\bfr,1;t_3}=F_{\bfr,1;t_4}=1+u_2 \hspace{0.9in} F_{\bfr,2;t_2}=F_{\bfr,2;t_3}=1+u_1+u_2 \hspace{0.9in} F_{\bfr,1;t_1}=F_{\bfr,1;t_2}=1+u_1.\]

  Alternatively, taking $\bfr=(1,3)$, we get $Z_1(u)=1+u$ and $Z_2(u)=1+z_1u+z_2u^2+u^3$, where we write $z_j$ instead of $z_{2,j}$ for $j=1,2$.
  This example exhibits an 8-fold periodicity in the sense of Definition~\ref{def:periodicity} with $\sigma$ the identity permutation of the set $\{1,2\}$.
  In particular, to compute all the $x_{j;t}$ and $y_{\ell;t}$ it suffices to consider a portion of the exchange tree $\TT_n$ as below:
  \[t_0 \stackrel{1}{\longleftrightarrow} t_1 \stackrel{2}{\longleftrightarrow} t_2 \stackrel{1}{\longleftrightarrow} t_3 \stackrel{2}{\longleftrightarrow} t_4 \stackrel{1}{\longleftrightarrow} t_5 \stackrel{2}{\longleftrightarrow} t_6 \stackrel{1}{\longleftrightarrow} t_7 \stackrel{2}{\longleftrightarrow} t_8,\]
  here $t_8$ can be identified with $t_0$ by periodicity.
  The $C$-matrices and $G$-matrices can be computed as:
  \begin{align*}
    &C_{t_0}=\left[\begin{array}{cc} 1 & 0\\ 0 & 1\end{array}\right] &&
    G_{t_0}=\left[\begin{array}{cc} 1 & 0\\ 0 & 1\end{array}\right] &&
    C_{t_1}=\left[\begin{array}{cc} -1 & 1\\ 0 & 1\end{array}\right] &&
    G_{t_1}=\left[\begin{array}{cc} -1 & 0\\ 1 & 1\end{array}\right] \\
    &C_{t_2}=\left[\begin{array}{cc} 2 & -1\\ 3 & -1\end{array}\right] &&
    G_{t_2}=\left[\begin{array}{cc} -1 & -3\\ 1 & 2\end{array}\right] &&
    C_{t_3}=\left[\begin{array}{cc} -2 & 1\\ -3 & 2\end{array}\right] &&
    G_{t_3}=\left[\begin{array}{cc} -2 & -3\\ 1 & 2\end{array}\right] \\
    &C_{t_4}=\left[\begin{array}{cc} 1 & -1\\ 3 & -2\end{array}\right] &&
    G_{t_4}=\left[\begin{array}{cc} -2 & -3\\ 1 & 1\end{array}\right] &&
    C_{t_5}=\left[\begin{array}{cc} -1 & 0\\ -3 & 1\end{array}\right] &&
    G_{t_5}=\left[\begin{array}{cc} -1 & -3\\ 0 & 1\end{array}\right] \\
    &C_{t_6}=\left[\begin{array}{cc} -1 & 0\\ 0 & -1\end{array}\right] &&
    G_{t_6}=\left[\begin{array}{cc} -1 & 0\\ 0 & -1\end{array}\right] &&
    C_{t_7}=\left[\begin{array}{cc} 1 & 0\\ 0 & -1\end{array}\right] &&
    G_{t_7}=\left[\begin{array}{cc} 1 & 0\\ 0 & -1\end{array}\right] 
  \end{align*}
  The cluster variables can then be computed from the separation of additions formulas \eqref{eq:separation of additions 1} and \eqref{eq:separation of additions 2} using the $F$-polynomials below:
  \begin{align*}
    F_{\bfr,1;t_7}=F_{\bfr,1;t_0}&=1\\
    F_{\bfr,2;t_0}=F_{\bfr,2;t_1}&=1\\
    F_{\bfr,1;t_1}=F_{\bfr,1;t_2}&=1+u_1\\
    F_{\bfr,2;t_2}=F_{\bfr,2;t_3}&=1+3u_1+3u_1^2+u_1^3+z_1u_1u_2+2z_1u_1^2u_2+z_1u_1^3u_2+z_2u_1^2u_2^2+z_2u_1^3u_2^2+u_1^3u_2^3\\
    F_{\bfr,1;t_3}=F_{\bfr,1;t_4}&=1+2u_1+u_1^2+z_1u_1u_2+z_1u_1^2u_2+z_2u_1^2u_2^2+u_1^2u_2^3\\
    F_{\bfr,2;t_4}=F_{\bfr,2;t_5}&=1+3u_1+3u_1^2+u_1^3+2z_1u_1u_2+4z_1u_1^2u_2+2z_1u_1^3u_2+z_2u_1u_2^2+z_1^2u_1^2u_2^2+3z_2u_1^2u_2^2\\
    &\quad+z_1^2u_1^3u_2^2+2z_2u_1^3u_2^2+3u_1^2u_2^3+z_1z_2u_1^2u_2^3+2u_1^3u_2^3+2z_1z_2u_1^3u_2^3+z_1u_1^2u_2^4+2z_1u_1^3u_2^4\\
    &\quad+z_2^2u_1^3u_2^4+2z_2u_1^3u_2^5+u_1^3u_2^6\\
    F_{\bfr,1;t_5}=F_{\bfr,1;t_6}&=1+u_1+z_1u_1u_2+z_2u_1u_2^2+u_1u_2^3\\
    F_{\bfr,2;t_6}=F_{\bfr,2;t_7}&=1+z_1u_2+z_2u_2^2+u_2^3.\\
  \end{align*}
  For example, the type-$\cX$ cluster variables $y_{1;t_3}$ and $y_{2;t_3}$ are given by
  \begin{align*}
    y_{1;t_3}
    &=y_{1;t_0}^{-2}y_{2;t_0}^{-3}\big(1+3y_{1;t_0}+3y_{1;t_0}^2+y_{1;t_0}^3+z_1y_{1;t_0}y_{2;t_0}+2z_1y_{1;t_0}^2y_{2;t_0}\\
    &\hspace{1in}+z_1y_{1;t_0}^3y_{2;t_0}+z_2y_{1;t_0}^2y_{2;t_0}^2+z_2y_{1;t_0}^3y_{2;t_0}^2+y_{1;t_0}^3y_{2;t_0}^3\big)\\
    y_{2;t_3}
    &=y_{1;t_0}y_{2;t_0}^2\big(1+2y_{1;t_0}+y_{1;t_0}^2+z_1y_{1;t_0}y_{2;t_0}+z_1y_{1;t_0}^2y_{2;t_0}+z_2y_{1;t_0}^2y_{2;t_0}^2+y_{1;t_0}^2y_{2;t_0}^3\big)^{-1}
  \end{align*}
  while, using that $\hat y_{1;t_0}=x_{2;t_0}^{-1}$ and $\hat y_{2;t_0}=x_{1;t_0}$, the type-$\cA$ cluster variables $x_{1;t_3}$ and $x_{2;t_3}$ are given by
  \begin{align*}
    x_{1;t_3}
    &=x_{1;t_0}^{-2}x_{2;t_0}\big(1+2x_{2;t_0}^{-1}+x_{2;t_0}^{-2}+z_1x_{2;t_0}^{-1}x_{1;t_0}+z_1x_{2;t_0}^{-2}x_{1;t_0}+z_2x_{2;t_0}^{-2}x_{1;t_0}^2+x_{2;t_0}^{-2}x_{1;t_0}^3\big)\\
    &=x_{1;t_0}^{-2}x_{2;t_0}^{-1}\big(x_{2;t_0}^2+2x_{2;t_0}+1+z_1x_{1;t_0}x_{2;t_0}+z_1x_{1;t_0}+z_2x_{1;t_0}^2+x_{1;t_0}^3\big)\\
    x_{2;t_3}
    &=x_{1;t_0}^{-3}x_{2;t_0}^2\big(1+3x_{2;t_0}^{-1}+3x_{2;t_0}^{-2}+x_{2;t_0}^{-3}+z_1x_{2;t_0}^{-1}x_{1;t_0}+2z_1x_{2;t_0}^{-2}x_{1;t_0}\\
    &\hspace{1in}+z_1x_{2;t_0}^{-3}x_{1;t_0}+z_2x_{2;t_0}^{-2}x_{1;t_0}^2+z_2x_{2;t_0}^{-3}x_{1;t_0}^2+x_{2;t_0}^{-3}x_{1;t_0}^3\big)\\
    &=x_{1;t_0}^{-3}x_{2;t_0}^{-1}\big(x_{2;t_0}^3+3x_{2;t_0}^2+3x_{2;t_0}+1+z_1x_{1;t_0}x_{2;t_0}^2+2z_1x_{1;t_0}x_{2;t_0}\\
    &\hspace{1in}+z_1x_{1;t_0}+z_2x_{1;t_0}^2x_{2;t_0}+z_2x_{1;t_0}^2+x_{1;t_0}^3\big).\\
  \end{align*}
\end{example}
...
\begin{enumerate}
  \item We lift the Poisson ensemble map $\rho:L_\cA\to L_\cX$ in \eqref{eq:ensemble} to symplectic groupoid comorphisms.
    When the exchange matrix is square (and hence invertible), the Poisson ensemble map is a diffeomorphism from its domain onto its image and so the graphs of the groupoid comorphisms also define groupoid morphisms over suitable loci.
...
\begin{remark}
  ...
  Since we begin with a Poisson structure on $L_\cA$ compatible with mutations, when $m=n$ the exchange matrix $\tilde B$ is invertible and hence $\rho:L_\cA\to L_\cX$ is a birational isomorphism.
  It follows that the Poisson ensemble map $\rho$ actually lifts to true morphisms of symplectic groupoids $\rho:\cG_\cA\to\cG_\cX$, $\rho:\cB_\cA\to\cB_\cX$, and $\rho:\cD_\cA\to\cD_\cX$ given respectively on fiber coordinates by 
  \begin{align*}
  ...
  \end{align*}
  Observe that care must me taken when considering the fractional exponents above in the complex case, however the maps are in fact well-defined.
  Indeed, a morphism of groupoids must take the identity image to the identity image and thus there is a unique choice of branch cut to take in these seemingly multi-valued functions, in other words the maps $\rho:\cB_\cA\to\cB_\cX$ and $\rho:\cD_\cA\to\cD_\cX$ are actually well-defined covering maps on the fibers.
...
\begin{proof}
  ...
  To ease the notation, we fix $\bfr$ and drop all subscripts and superscripts.
...
\begin{proof}
  ...
  Now suppose $t'$ and $t$ are joined by an edge labeled $k$ with $t$ lying on the unique shortest path in $\TT_n$ from $t_0$ to $t'$.
  ...
  \end{align*}
  where the final exponent above is given by \eqref{eq:matrix mutation}; by skew-symmetrizability this exponent may be rewritten as $B_{i\ell;t'}/d_\ell-B_{ik;t} [-\varepsilon B_{\ell k;t} r_k]_+/d_k$.
...
\begin{example}
  Continuing from Example~\ref{ex:running 3} in the case $\bfr=(1,3)$, we see from Lemma~\ref{le:dual vectors} that $C_t^\vee=C_t^T$ and $G_t^\vee=G_t^T$ for $t\in\TT_n$, where $(-)^T$ denotes the transpose of a matrix.
  The iterated groupoid mutations can thus be computed from the data given in Example~\ref{ex:running 3} using the separation of additions formulas in Theorem~\ref{th:groupoid separation of additions}.
  For example, the cluster groupoid variable $t_{1;t_3}$ is given by
  \begin{align*}
    t_{1;t_3}
    &=t_{1;t_0}^{-2}t_{2;t_0}^{-1}\frac{t_{2;t_0}^2+2y_{1;t_0}t_{2;t_0}+y_{1;t_0}^2+z_1y_{1;t_0}y_{2;t_0}t_{1;t_0}t_{2;t_0}+z_1y_{1;t_0}^2y_{2;t_0}t_{1;t_0}+z_2y_{1;t_0}^2y_{2;t_0}^2t_{1;t_0}^2+y_{1;t_0}^2y_{2;t_0}^3t_{1;t_0}^3}{1+2y_{1;t_0}+y_{1;t_0}^2+z_1y_{1;t_0}y_{2;t_0}+z_1y_{1;t_0}^2y_{2;t_0}+z_2y_{1;t_0}^2y_{2;t_0}^2+y_{1;t_0}^2y_{2;t_0}^3}.
  \end{align*}
\end{example}
