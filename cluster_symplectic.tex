...
For $1\le k\le n$ and a sign $\varepsilon\in\{\pm1\}$, let $E_i{k,\varepsilon}$ be the $m\times m$ matrix with entries
...
\begin{example}
  ...
  Given $\tilde B'$ mutation equivalent to $\tilde B_{prin}$, the lower $n\times n$ submatrix $C$ of $\tilde B'$ has the following remarkable \emph{sign-coherence} property: each column of $C$, known as a $\bfc$-vector, has either all non-negative entries or all non-positive entries \cite{fomin-zelevinsky4,nakanishi-zelevinsky,gross-hacking-keel-kontsevich}.
  Thus each seed $\Sigma'$ mutation equivalent to $\Sigma_{prin}$ admits a collection of \emph{tropical signs} $\varepsilon_k\in\{\pm1\}$, where $\varepsilon _k=1$ if the entries in the $k$-th column of $C$ are non-negative and $\varepsilon_k=-1$ if the entries in the $k$-th column of $C$ are non-positive. 
\end{example}

\begin{remark}
  Given an arbitrary exchange matrix $\tilde B$ and an exchange matrix $\tilde B'$ mutation equivalent to $\tilde B$, we associate the same tropical signs $\varepsilon_k$ to the columns of $\tilde B'$.
\end{remark}
...
\begin{theorem}
  ...
  Let $\Sigma$ be a seed in $\cF$ and $\Sigma'\sim\Sigma$.
  Each cluster variable $x'_i$ of $\Sigma'$ is an element of the subring $\ZZ[x_1^{\pm1},\ldots,x_m^{\pm1}]\subset\cF$.
\end{theorem}
