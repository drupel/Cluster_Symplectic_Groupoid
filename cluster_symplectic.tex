  ...
  The Poisson ensemble map $\rho:L_\cA\to L_\cX$ \eqref{eq:ensemble} lifts to the following comorphisms of symplectic groupoids.
	\begin{enumerate}
		...
		\begin{equation} \label{eq:ensembleG}
			P^*\left(\sum_{k=1}^n B_{ik} y_k q_k\right) = x_i p_i
		\end{equation}
		\item $(\rho, P)$ from $\cB_\cA$ to $\cB_\cX$ such that
		\[
			P^*\left(\prod_{k=1}^n (v_iy_i+1)^{B_{ik}} \right) = u_ix_i+1
		\]
		\item $(\rho, P)$ from $\cD_\cA$ to $\cD_\cX$ such that
		\[
			P^*\left(\prod_{k=1}^n t_i^{B_{ik}} \right) = s_i
		\]
	\end{enumerate}
\end{lemma}
\begin{proof}
	...
	\[
		dy_k = \sum_{i=1}^m \frac{B_{ik}}{x_i} y_k dx_i, \qquad k = 1, \ldots, n.
	\]
The pullback of $1$-forms is
	\[
		\sum_{k=1}^n q_k dy_k = \sum_{i=1}^m \frac{1}{x_i} \sum_{k=1}^n B_{ik} y_k q_k dx_i.
	\]
or as a bundle map
	\[
		\rho^*: \rho^! T^*_{\pi_\cX} L_\cX \to T^*_{\pi_\cA} L_\cA, \qquad (\rho^*)^*(p_i) = \frac{1}{x_i} \sum_{k=1}^n B_{ik} y_k q_k, \qquad i = 1, \ldots, m.
	\]
