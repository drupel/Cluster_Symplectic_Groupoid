...
If $(\cG_1, \omega_1) \rra (M_1, \pi_1)$ is the source-simply-connected symplectic groupoid of $(M_1, \pi_1)$ and $(\cG_2, \omega_2) \rra (M_2, \pi_2)$ is any symplectic groupoid of $(M_2, \pi_2)$, then a Poisson map $\varphi: (M_1, \pi_1) \to (M_2, \pi_2)$ lifts to a symplectic groupoid map $\tilde{\varphi}: (\cG_1, \omega_1) \rra (\cG_2, \omega_2)$. The map $\tilde{\varphi}$ is surjective if $\varphi$ is surjective and $\cG_2$ is source-connected. However in general Poisson maps do necessarily lift to symplectic groupoid maps. This will be a recurring theme when we lift to cluster mutations to groupoid mutations.
...
\begin{proposition}
  As symplectic groupoids over the log Poisson structure on $\RR+^m$ or $(\CC^\times)^m$, the map
  \[
...
\begin{corollary}
  ...
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, the time-$t$ flow $\varphi_\cX^t:L_\cX\to L_\cX$ of the vector field $X_\cX^{k,\varepsilon}$ given on coordiates by
\[
	 (\varphi_\cX^t)^*(y_\ell)=(1+y_k^\varepsilon)^{-tB_{k\ell}}y_\ell
\]
and the time-$t$ flow $\varphi_{X_\cA}^t:L_\cA\to L_\cA$ of the vector field $X_\cA^{k,\varepsilon}$ given on coordiates by
\[
    (\varphi_\cA^t)^*(x_j)=(1+\hat y_k^\varepsilon)^{-t\delta_{jk}}x_j.
\]
preserves the respective Poisson structures.
\end{corollary}
\begin{proof}
...
To lift the Poisson maps $\tau_\cX^{k,\varepsilon}:L_\cX\to L'_\cX$ and $\tau_\cA^{k,\varepsilon}:L_\cA\to L'_\cA$ to groupoid maps, we use their Hamiltonian nature. In general Poisson symmetries are lifted to symplectic groupoid symmetries and Hamiltonian symmetries are lifted to Hamiltonian groupoid symmetries \cite{MR2504211}. More precisely, if $(\cG, \omega) \rra (M, \pi)$ is a symplectic groupoid and $X_f \in \cT_M$ is the Hamiltonian vector field of $f \in \cO_M$, then $X_f$ is lifted to a Hamiltonian vector field $X_{\tilde{f}} \in \cT_\cG$ where $\tilde{f} = \alpha^*f - \beta^*f$. Indeed $\tilde{f} \in \cO_\cG$ is multiplicative since $\tilde{f} = \alpha^*f - \beta^*f = \partial f$, where $\partial$ is the groupoid coboundary operator \eqref{eq:gpdcob}, and $\omega$ is multiplicative by definition, so the Hamiltonian vector field $X_{\tilde{f}} = \iota_{d\tilde{f}}\omega^{-1}$ is compatible with the groupoid structures. That is, the flow of $X_{\tilde{f}}$ preserves the symplectic groupoid structures on $(\cG, \omega)$.
...
\]
\begin{corollary}
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, the Poisson autmorphism $\varphi_\cX^1: L_\cX \to L_\cX$ in Corollary~\ref{cor:time-one flows} is lifted  to the symplectic groupoid automorphisms $\tilde\varphi_\cX^1: \cG_\cX \to \cG_\cX$ and $\bar\varphi_\cX^1: \cD_\cX \to \cD_\cX$, which on coordinates are given by
  \begin{align*}
  ...
  \end{align*}
and the Poisson autmorphism $\varphi_\cA^1: L_\cX \to L_\cX$ Corollary~\ref{cor:time-one flows} is lifted  to the symplectic groupoid automorphisms $\tilde\varphi_\cA^1: \cG_\cA \to \cG_\cA$ and $\bar\varphi_\cA^1: \cD_\cA \to \cD_\cA$, which on coordinates are given by
  \begin{align*}
  ...
  \end{align*}
\end{corollary}
\footnote{time-$t$ flow and lifting the flow of the vector field on the base.}
...
\begin{equation} \label{eq:GpdCD}
	\xymatrix{
		& \cD_\cA \ar@/_1pc/[dd]|-(.2){\bar{\rho}} \ar[rr]|-{\bar{\tau}_\cA} \ar@/^1.2pc/[rrrr]|-{\bar{\mu}_\cA} && \cD_\cA \ar@/_1pc/[dd]|-{\bar{\rho}} \ar[rr]|-{\bar{\varphi}_\cA} && \cD'_\cA \ar@/_1pc/[dd]|-{\bar{\rho}} \\
	\cG_\cA \ar@/_1pc/[dd]|-{\tilde{\rho}} \ar[rr]|-{\tilde{\tau}_\cA} \ar[ur]|-{\kappa_\cA} \ar@/^1.2pc/[rrrr]|-{\tilde{\mu}_\cA} && \cG_\cA \ar@/_1pc/[dd]|-{\tilde{\rho}} \ar[rr]|-{\tilde{\varphi}_\cA} \ar[ur]|-{\kappa_\cA} && \cG'_\cA \ar@/_1pc/[dd]|-{\tilde{\rho}} \ar[ur]|-{\kappa_\cA} \\
		& \cD_\cX \ar[rr]|-{\bar{\tau}_\cX} \ar@/_1.2pc/[rrrr]|-{\bar{\mu}_\cX} && \cD_\cX \ar[rr]|-{\bar{\varphi}_\cX} && \cD'_\cX \\
