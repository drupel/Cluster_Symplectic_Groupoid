...
\begin{remark}
  Combining the periodicity result from Proposition~\ref{prop:gpd periodicity} and the iterated mutation formula~\eqref{eq:cotangent mutations} for $T^* L_\bfr$, we obtain a collection of identities, one for each $1\le\ell\le n$, each choice of $1\le j\le r_\ell$, and any periodicity $(t_1,\ldots,t_w;\sigma)$ of cluster mutations.
  Taken together these identities roughly imply that the left-hand side of the Roger's dilogarithm identity \cite[Equation (4.28)]{Nak16} is independent of the choice of exchange polynomial coefficients $z_{\ell,j}$.
