...
Consider a compatible pair of matrices $(\tilde B,\Omega)$ with $\tilde B$ an $m\times n$ matrix and satisfying $\tilde B^T\Omega=[D\ 0]$.
...
Given a choice of sign $\varepsilon\in\{\pm1\}$, we define Hamiltonian functions $H_{\cX,k,\varepsilon}\in\cO_{L_\cX}$ and $H_{\cA,k,\varepsilon}\in\cO_{L_\cA}$ by
\begin{equation}
...
\begin{lemma}
  ...
  \begin{align}
    \label{eq:X dynamics}
    \dot y_\ell&:=\{H_{\cX,k,\varepsilon},y_\ell\}_\cX=-B_{k\ell}\log(1+y_k^\varepsilon)y_\ell;\\
    \label{eq:A dynamics}
    \dot x_j&:=\{H_{\cA,k,\varepsilon},x_j\}_\cA=-\delta_{jk}\log(1+\hat y_k^\varepsilon)x_j.
  \end{align}
\end{lemma}
