...
Our first observation is that, despite the warnings of Remark~\ref{rem:lifting}, the Poisson ensemble map $\rho:L_\cA\to L_\cX$ lifts to maps between each of the symplectic groupoids over these spaces.
\begin{lemma}
  The Poisson ensemble map $\rho:L_\cA\to L_\cX$ lifts to morphisms of symplectic groupoids $\rho:\cG_\cA\to\cG_\cX$, $\rho:\cB_\cA\to\cB_\cX$, and $\rho:\cD_\cA\to\cD_\cX$ given on fiber coordinates by 
  \begin{align*}
...
\end{proof}
\begin{remark}
  While the formulae for the flows above were defined when $L^+_\cX\cong\RR_+^n$ and $L^+_\cA\cong\RR_+^m$, the formulae for the time-one flows on $\cG_\cX$ and $\cG_\cA$ can be applied equally well in the settings $L^+_\cX\cong(\CC^\times)^n$ and $L^+_\cA\cong(\CC^\times)^m$.
...
\begin{lemma}
  ...
  \begin{align*}
    \tau_\cX^{k,\varepsilon}&:\cG_\cX\to\cG'_\cX,& \tau_\cX^{k,\varepsilon}&:\cB_\cX\to\cB'_\cX,& \tau_\cX^{k,\varepsilon}&:\cD_\cX\to\cD'_\cX\\
    \tau_\cA^{k,\varepsilon}&:\cG_\cA\to\cG'_\cA,& \tau_\cA^{k,\varepsilon}&:\cB_\cA\to\cB'_\cA,& \tau_\cA^{k,\varepsilon}&:\cD_\cA\to\cD'_\cA
  \end{align*}
  lifting respectively the Poisson morphisms $\tau_\cX^{k,\varepsilon}:L^+_\cX\to L^+_{\cX'}$ and $\tau_\cA^{k,\varepsilon}:L^+_\cA\to L^+_{\cA'}$ of Lemma~\ref{le:tropical cluster transformations}.
...
\begin{definition}
  Fix an initial $m\times n$ exchange matrix $\tilde B^{t_0}$ and an initial skew-symmetric matrix $\Omega^{t_0}$ which is $D$-compatible with $\tilde B^{t_0}$.
...
Given a pair of vertices $t,t'\in\TT_n$, we follow the notation of Section~\ref{sec:cluster} and write $\mu_{\cX,\bfr}^{t,t'}$ and $\mu_{\cA,\bfr}^{t,t'}$ for each of the composed groupoid mutations over ithe cluster charts of $\cX_\bfr$ and $\cA_\bfr$ respectively.
In particular, we have the following analogues of the separation of additions formulas describing the cluster coordinates.
\begin{theorem}
