...
If we restrict $L$ to be either $\RR_+^m$ or $(\CC^*)^m$ where $\RR_+ = (0, \infty)$ and $\CC^* = \CC \setminus \{0\}$, then there is another interesting source-connected symplectic groupoid of the log canonical Poisson space $(L, \pi)$, called the symplectic double \cite{MR2470108}.
...
{\color{red} double-check the groupoid structures. Also expend the remark on the notations.}

Comparing to \cite{MR2470108}, we have chosen different notations so that the groupoid covering map from the source-simply-connected $\cG_L \rra L$ to the symplectic double $\cD_L \rra L$ has a simple expression.

\begin{proposition}
The covering map $\varphi: \cG_L \to \cD_L$ given by $\varphi (\bfp, \bfx) = (e^{x_1p_1}, \ldots, e^{x_mp_m}, \bfx)$ is a groupoid morphism.
\end{proposition}
