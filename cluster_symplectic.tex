...
\begin{remark}
For the standard symplectic structure $\omega_0$ in Theorem~\ref{thm:poissp}, we use the sign convention that $\omega_0 = - d \theta_0$ where $\theta_0$ is the tautological 1-form on $T^*M$. This choice ensures that the source map $\alpha$ is Poisson and the target map $\beta$ is anti-Poisson, instead of the other way around. In coordinates, we have $\omega_0 = \sum_i d x_i \wedge d p_i$.
\end{remark}
