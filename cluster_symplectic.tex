...
We begin in Section~\ref{sec:cluster} in the setting of generalized cluster mutations $\mu: L \to L'$ which we decompose into two maps $\varphi^1$ and $\tau$ according to the Hamiltonian perspective of mutations \cite{FG09a, GNR17}.
...
%In this section, we will use the idea of Poisson spray \cite{CM11} to construct the source-simply-connected sympletic groupoid of a log-canonical Poisson structure.
...
%Both $\cG \rra L$ and $\cB \rra L$ are symplectic groupoids over $L$, and as we will see in \S\ref{sec:groupoid mutations}, this enables us to define the groupoid mutations for $\cG$ and $\cB$ over $\bar{L}^\times$ when the underlying field is $\RR$.
...
\begin{remark}
  ...
  We will refer to this as the ``cluster case'' since the mutations below reduce to the classical mutation rules for cluster algebras (c.f. Remark~\ref{cluster case 2}).
\end{remark}
...
Let $L_{\cX'}=\RR_\times^n$ and $L_{\cA'}=\RR_\times^m$ with coordinates $\bfy'=(y'_1,\ldots,y'_n)$ and $\bfx'=(x'_1,\ldots,x'_m)$ respectively.
...
\begin{theorem}
  \cite{GSV10}
  The Poisson structures on the cluster charts $L^\times_{\cX,\bfr;t}$ and $L^\times_{\cA,\bfr;t}$ determined by the $D$-compatible pairs $(\tilde B_t,\Omega_t)$ glue to give a global Poisson structure on the cluster varieties $\cX_\bfr$ and $\cA_\bfr$.
...
Instead a Poisson map induces a Lie algebroid comorphism.
...
The Poisson map $\varphi: (M_1, \pi_1) \to (M_2, \pi_2)$ induces the Lie algebroid comorphism $\varphi^*: \varphi^!  T^*_{\pi_2} M_2 \to T^*_{\pi_1} M_1$, which means that the following diagram commutes \cite{HM90}.
\begin{equation} 
	\xymatrix{
...
\end{equation}
This comorphsim $\varphi^*$ is complete if for a complete Hamiltonian vector field $X_f$ with $f \in \cO_{M_2}$, the pullback Hamiltonian vector field $X_{\varphi^*f}$ is also complete.
...
\begin{enumerate}
  \item We lift the Poisson ensemble map $\rho:L_\cA\to L_\cX$ \eqref{eq:ensemble} to symplectic groupoid comorphisms.
    When the exchange matrix is square, the graph of each groupoid comorphism also defines a groupoid morphism.
...
\begin{align*}
  \cG_\cX&\cong\RR^n\times L^\times_\cX,& \cB_\cX&\cong\RR^n\times L^\times_\cX,& \cD_\cX&\cong\RR_\times^n\times L^\times_\cX,\\
  \cG_\cA&\cong\RR^m\times L^\times_\cA,& \cB_\cA&\cong\RR^m\times L^\times_\cA,& \cD_\cA&\cong\RR_\times^m\times L^\times_\cA.
\end{align*}
...
\end{itemize}
The symplectic groupoid structure on $\cG_\cA \rra L^\times_\cA$ is the same as in Theorem~\ref{thm:PoiSpLogC}, while the symplectic groupoid structure on $\cD_\cA \rra L^\times_\cA$ is the same as in Theorem~\ref{thm:SymDBLoc}, and the symplectic groupoid structure on $\cB_\cA \rra L^\times_\cA$ is the same as in Theorem~\ref{th:blowup groupoid}.
The symplectic groupoid structures over $L^\times_\cX$ mimic the structures in the theorems above by replacing $x_i$ with $y_k$ and $\Omega_{ij}$ with $d_kB_{k\ell}$ together with the corresponding replacements of coordinates $p_j$ with $q_\ell$, $u_j$ with $v_\ell$, and $s_j$ with $t_\ell$.
...
\begin{lemma} \label{lemma:ensemblegpd}
  The Poisson ensemble map $\rho:L^\times_\cA\to L^\times_\cX$ \eqref{eq:ensemble} lifts to the following comorphisms of symplectic groupoids:
  \begin{enumerate}
...
\end{lemma}
\begin{proof}
  We prove \eqref{eq:ensembleG}, the formulae for $\cB$ and $\cD$ follow easily using the diagram \eqref{eq:LGpdCD}.
...
\begin{remark}
  ...
  When $m=n$, i.e.\ the map $\rho:L_\cA\to L_\cX$ is an isomorphism, the Poisson ensemble map actually lifts to true morphisms of symplectic groupoids $\rho:\cG_\cA\to\cG_\cX$, $\rho:\cB_\cA\to\cB_\cX$, and $\rho:\cD_\cA\to\cD_\cX$ given respectively on fiber coordinates by 
  \begin{align*}
  ...
  The choice of coordinates used in \cite{FG09c} can be identified with our coordinates $t_\ell^{d_\ell}$, one may motivate such a choice by desiring to avoid the fractional exponents above, however as observed such considerations are unnecessary.
...
\begin{lemma}
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, the Hamiltonian vector fields $X_{\cX,\bfr}^{k,\varepsilon}$ determine the following dynamics on the symplectic groupoids $\cG_{\cX,\bfr}$, $\cB_{\cX,\bfr}$, $\cD_{\cX,\bfr}$ respectively:
  \begin{align}
  ...
  \end{align}
  and the Hamiltonian vector fields $X_{\cA,\bfr}^{k,\varepsilon}$ determine the following dynamics on the symplectic groupoids $\cG_{\cA,\bfr}$, $\cB_{\cA,\bfr}$, $\cD_{\cA,\bfr}$ respectively:
  \begin{align}
    \label{eq:p dot}
...
\begin{corollary}
  ...
  \end{align*}
  and the Hamiltonian flow $\varphi_{\cA,\bfr}^t: L^\times_{\cA,\bfr} \to L^\times_{\cA,\bfr}$ in Corollary~\ref{cor:time-one flows} is lifted to the multiplicative Hamiltonian flows $\varphi_{\cA,\bfr}^t: \cG_{\cA,\bfr} \to \cG_{\cA,\bfr}$, $\varphi_{\cA,\bfr}^t: \cB_{\cA,\bfr} \to \cB_{\cA,\bfr}$, $\varphi_{\cA,\bfr}^t: \cD_{\cA,\bfr} \to \cD_{\cA,\bfr}$, given on coordinates by
  \begin{align*}
...
That is, over each cluster chart $L^\times_{\cX,\bfr;t}$ and $L^\times_{\cA,\bfr;t}$ we have, respectively, groupoids $\cG_{\cX,\bfr;t}$, $\cB_{\cX,\bfr;t}$, $\cD_{\cX,\bfr;t}$ and $\cG_{\cA,\bfr;t}$, $\cB_{\cA,\bfr;t}$, $\cD_{\cA,\bfr;t}$ with all of the structure as above.

That is, over each cluster chart $L^\times_{\cX,\bfr;t}$ and $L^\times_{\cA,\bfr;t}$ we have, respectively, groupoids $\cG_{\cX,\bfr;t}$, $\cB_{\cX,\bfr;t}$, $\cD_{\cX,\bfr;t}$ and $\cG_{\cA,\bfr;t}$, $\cB_{\cA,\bfr;t}$, $\cD_{\cA,\bfr;t}$ with all of the structure as above associated to the pairs $(\tilde B_t,\Omega_t)$ for each $t\in\TT_n$.
\begin{definition}
...
\begin{remark}
  ...
  Then the compatibility of $\{\cdot,\cdot\}$, together with the exchange relations, imposes the compatibility condition $\tilde B^T\Omega=[D\ 0]$ (see \cite{GSV03,BZ05} for details).
\end{remark}

\begin{theorem}
  \cite{GSV03}
  Suppose the $m\times n$ exchange matrix $\tilde B$ of a seed $\Sigma$ has full rank.
  Then there exists a Poisson structure $\Omega$ compatible with the cluster structure on $\cA(\Sigma)$.
\end{theorem}
...
%Write $\pi$ for the corresponding Poisson bi-vector on the cluster manifold $M$, i.e.\ in local coordinates on $L_\Sigma$ we have $\pi=\sum\limits_{i>j}\Omega_{ij}x_ix_j\frac{\partial}{\partial x_i}\wedge\frac{\partial}{\partial x_j}$.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &=-\sum\limits_{j=1}^m \Omega_{kj} x_j p_j+\ln\left(\bfs^{[\bfb_k]_+}\right)+\ln\left(\frac{Q_k(\bfs\circ\bfx)}{Q_k(\bfx)}\right)\\
