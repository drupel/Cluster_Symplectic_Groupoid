...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item $\big(\mu_{-,\bfr}^{t_w,t_1}\big)^*(z_{\ell,j;t_w})=z_{\sigma(\ell),j;t_1}$ for $1\le j\le m$;
...
\begin{theorem}
  ...
  \begin{align*}
    ...
    &=\begin{cases}
      a_{k,r_k-j}+\frac{\varepsilon}{d_k}\int_{y_k^\varepsilon}^{\beta^*(y_k^\varepsilon)} \frac{u^{\varepsilon (r_k-j)-1}}{Z_k(u^\varepsilon)}du & \text{if $\ell=k$;}\\
      a_{\ell,j} & \text{if $\ell\ne k$.}
    \end{cases}\\
    (\mu_{\cA,\bfr}^{k,\varepsilon})^*(a'_{\ell,j})
    &=\begin{cases}
      a_{k,r_k-j}+\frac{\varepsilon}{d_k}\int_{\hat y_k^\varepsilon}^{\beta^*(\hat y_k^\varepsilon)} \frac{u^{\varepsilon (r_k-j)-1}}{Z_k(u^\varepsilon)}du & \text{if $\ell=k$;}\\
      a_{\ell,j} & \text{if $\ell\ne k$.}
    \end{cases}
  \end{align*}
...
If we restrict to the case of $\RR_\times$, then both the blow-up groupoid $\cB$ and the symplectic double $\cD$ are also source-simply-connected and parts (2), (3), (6) and (7) follows from Proposition~\ref{prop:gpd identity}; the case of $\CC_\times$ follows when we complexify the mutations. 

Now the periodicity of mutations depends only on the degree of $Z_k$; in other words. This means that if we fix the degree $r_l$ of $Z_l$ as in \eqref{eq:gen mut poly} and a periodicity $(t_1,\ldots,t_w;\sigma)$ for say $L_\cX$, then $\mu_\cX^{t_w,t_1}: L_{\cX} \to L_{\cX}$ is the identity for each choice of $(z_1, \ldots, z_{r_l-1}) \in \RR^{r_l - 1} \subset L_\bfr$, and therefore
\[
	\mu^{t_w,t_1}: L_\cX \times \RR^{r_l - 1} \to L_\cX \times \RR^{r_l - 1}
\]
is the identity map. By Proposition~\ref{prop:gpd identity}, $\mu^{t_w,t_1}$ lifts to the identity of the source-simply-connected symplectic groupoid and part (4) follows.
