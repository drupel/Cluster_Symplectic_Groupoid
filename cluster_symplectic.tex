...
The cluster $\cA$-varieties are the geometric realization of cluster algebras defined by Fomin and Zelevinsky \cite{MR1887642} as the culmination of their study of total positivity and canonical bases for algebraic groups.
...
The cluster $\cA$-varieties are often endowed with a class of compatible Poisson structures in the sense of Gekhtman, Shapiro, and Vainshtein \cite{MR2683456}.
...
For both the cluster $\cA$-varieties \cite{MR2683456} and $\cX$-varieties \cite{FG09c}, the compatible Poisson structures are log-canonical in local charts.
...
For each $k=1,\ldots,n$, fix a monic polynomial $F_k(z)\in\RR_+[z]$ with $F_k(0)=1$.
...
\end{equation}
Observe that when $F_k(z)=1+z$, we have $H_\cX^{k,\varepsilon}:=\frac{\varepsilon}{d_k}\Li_2(-y_k^\varepsilon)$ and $H_\cA^{k,\varepsilon}:=\frac{\varepsilon}{d_k}\Li_2(-\hat y_k^\varepsilon)$.

Clearly, there is an equality $\rho^*(H_\cX^{k,\varepsilon})=H_\cA^{k,\varepsilon}$.
Write $X_\cX^{k,\varepsilon}\in\cT_\cX$ and $X_\cA^{k,\varepsilon}\in\cT_\cA$ for the Hamiltonian vector fields associated to $H_\cX^{k,\varepsilon}$ and $H_\cA^{k,\varepsilon}$ respectively, i.e.\ the vector fields naturally associated to the derivations $\{H_\cX^{k,\varepsilon},\cdot\}_\cX$ and $\{H_\cA^{k,\varepsilon},\cdot\}_\cA$.
\begin{lemma}
  \label{le:hamiltonian dynamics}
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, the vector fields $X_\cX^{k,\varepsilon}$ and $X_\cA^{k,\varepsilon}$ determine the following dynamics on $L_\cX$ and $L_\cA$:
  \begin{align}
    \label{eq:X dynamics}
...
Denote by $r_k$ the degree of the exchange polynomial $F_k(z)$.
...
\[\mu_\cX^{k,\varepsilon}:=\tau_\cX^{k,\varepsilon}\circ\varphi_\cX^1:L_\cX\to L'_\cX\qquad\text{ and }\qquad\mu_\cA^{k,\varepsilon}:=\tau_\cA^{k,\varepsilon}\circ\varphi_\cA^1:L_\cA\to L'_\cA.\]
\begin{lemma}
...
\end{equation}
As observed above, the cluster mutations are independent of the choice of signs $\varepsilon$ while their automorphism and tropical parts are not.
In what follows, it will be important to make a particular choice of signs. 
To accomplish this, we observe that the mutation pattern gives rise to the following remarkable combinatorial construction.
\begin{example}
  ...
  Then, given $\tilde B^t$ mutation equivalent to $\tilde B^{t_0}$, the lower $n\times n$ submatrix $C^t$ of $\tilde B^t$ has the following remarkable \emph{sign-coherence} property: 
  \begin{itemize}
    \item each column of $C^t$, known as a $\bfc$-vector, has either all non-negative entries or all non-positive entries \cite{fomin-zelevinsky4,nakanishi-zelevinsky,gross-hacking-keel-kontsevich}.
  \end{itemize}
...
\begin{definition}
  ...
  The \emph{cluster varieties} \footnote{Why does this definition give something smooth?}
  $\cX=\cX_{\tilde B^{t_0}}$ and $\cA=\cA_{\tilde B^{t_0}}$ are obtained by gluing the cluster charts $L_{\cX;t}$ and $L_{\cA;t}$ respectively along the cluster mutations $\mu_\cX^{k,\varepsilon_{k;t}}$ and $\mu_\cA^{k,\varepsilon_{k;t}}$, that is
  \begin{equation}
    \label{eq:cluster varieties}
    \cX=\bigcup_{t\in\TT_n} L_{\cX;t}\qquad\text{ and }\cA=\bigcup_{t\in\TT_n} L_{\cA;t}.
  \end{equation}
\end{definition}

\begin{theorem}
  \cite{GSV13}
  The Poisson structures on the cluster charts $L_{\cX;t}$ and $L_{\cA;t}$ glue to give global Poisson structures on the cluster varieties $\cX$ and $\cA$.
\end{theorem}

The following result is the famous Laurent phenomenon for cluster algebras.
\begin{theorem}
  \cite{FZ01}
  Each cluster coordinate $x_{i;t}$ on the cluster chart $L_{\cA;t}$ determines a global function on $\cA$ which is given by a Laurent polynomial in the coordinates of any other cluster chart $L_{\cA;t'}$.
\end{theorem}
