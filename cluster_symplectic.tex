...
For both the cluster $\cA$-variety \cite{MR2683456} and $\cX$-variety \cite{MR2470108}, the compatible Poisson structures are log-canonical in local charts.  Using the results in \cite{MR2900786, CMS17}, we construct the source-simply-connected symplectic groupoid of a log-canonical Poisson structure by carefully chosing a Poisson spray. 
...
%		\varphi_X^t: (\bfp, \bfx) \mapsto \left(e^{-t\sum\limits_{i} \Omega_{1i} x_ip_i} p_1, \ldots, e^{-t\sum\limits_{i} \Omega_{mi} x_ip_i} p_m, e^{t\sum\limits_{i} \Omega_{1i} x_ip_i} x_1, \ldots, e^{t\sum\limits_{i} \Omega_{mi} x_ip_i} x_m\right)
	$$
where $a_i = e^{\sum\limits_{j} \Omega_{ij} x_jp_j}$, exists for all time $t \in \RR$ and contains no loops.
\end{lemma}
...
Our goal now is to apply the groupoid formalism to lift these results to the level of symplectic groupoids integrating the standard Poisson structures on $L_\cX$ and $L_\cA$.
