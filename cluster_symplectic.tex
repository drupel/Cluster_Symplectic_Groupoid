...
This is the first of a series of papers whose aim is to implement Weinstein's program of geometric quantization for Poisson manifolds \cite{MR1104934} in the case of cluster Poisson structures.
...
Indeed, this idea was implicitly implemented by Fock and Goncharov \cite{FG09c} in the case of $\cX$-varieties.
...
\begin{itemize}
  \item In Section~\ref{sec: local}, we build the symplectic groupoid in each local chart of a cluster variety.
    ...
    We also recall the symplectic double $\cD$ as in \cite{FG09c}, albeit with slightly different notations to accommodate the fact that we consider the non-square exchange matrices in this paper.
  ...
  \item In Section~\ref{sec:cluster}, we give the explicit formulae for the cluster groupoid mutation $\tilde{\mu}: \cG \to \cG'$.
    The first step is to decompose the cluster mutation $\mu: L \to L'$ into two maps $\varphi^1$ and $\tau$, where $\varphi^1$ is the time-$1$-flow of the Euler dilogarithm function and $\tau$ is a natural transformation of log-canonical coordinates.
    See \cite{FG09a} for the case of $\cX$-varieties and \cite{MR3691969} for the case of $\cA$-varieties.
...
In fact, $\varphi$ is Poisson means that its graph $\Gamma_\varphi \in M_1 \times M_2$ is coisotropic with respect to $\pi \oplus -\pi$ \cite{MR959095} and coisotropic submanifolds integrates to Lagrangian subgroupoids \cite{MR2104442}, so a Poisson map $\varphi: (M_1, \pi_1) \to (M_2, \pi_2)$ integrates to a Lagrangian subgroupoid of $(\cG_1\times\cG_2, \omega_1\oplus-\omega_2)$.
However, the Poisson maps occuring in cluster algebras always lift to symplectic groupoid maps.
This will be a recurring theme when we lift to cluster mutations to groupoid mutations.
...
\begin{remark}
	...
	With the notations in Theorem~\ref{thm:SymDBLoc} and the change of variables $e^{\chi_i} = x_i$ and $e^{\xi_i} = s_i$, the symplectic double $(\cD_L, \omega)$ becomes the symplectic vector space $(\cU, \omega)$ where $\cU$, with coordinates $(\xi_1, \ldots, \xi_m, \chi_1, \ldots, \chi_m)$, is either $\RR^{2m}$ or $\CC^{2m}$ and
	\[
		\omega = \sum_{i} d\chi_i \wedge d\xi_i + \sum_{j < i} \Omega_{ij} d\xi_i \wedge d\xi_j.
	\]
Although $(\cU, \omega)$ is not a symplectic groupoid of $(L, \pi)$ (e.g. the identity map cannot be defined), the source and target maps are well-defined in these coordinates:
	\begin{align*}
	...
	\end{align*}
The $\alpha$-fibers are symplectic orthogonal to the $\beta$-fibers; both $\alpha: (\cU, \omega) \to (L, \pi)$ and $\beta: (\cU, - \omega) \to (L, \pi)$ are Poisson maps.
...
For the standard symplectic structure $\omega_0 = \sum_{i} d\chi_i \wedge d\xi_i$ on $\cU$, since
	...
	\]
is supported on the $\alpha$-fiber, it follows that $\beta_*(\omega_0^{-1}) = \beta_*(\omega^{-1}) = -\pi$. The symplectic realization $\beta: (\cU, -\omega_0) \to (L, \pi)$ is the central object of study in \cite{MR3691969}.
...
We fix a skew-symmetrizing matrix $D$ and refer to an $m\times m$ matrix $\Omega=(\Omega_{ij})$ as \emph{$D$-compatible} with $\tilde B$ if $\tilde B^T\Omega=[D\ \boldsymbol{0}]$, where $\boldsymbol{0}$ denotes an $n\times(m-n)$ matrix with all zero entries.
...
Fix a collection of positive integers $\bfr=(r_1,\ldots,r_n)\in\ZZ_{>0}^n$ and write $L_\bfr=\prod_{k=1}^n \RR_+^{r_k-1}$.
Set 
\[L_{\cX,\bfr}:=L_\cX\times L_\bfr\qquad\text{ and }\qquad L_{\cA,\bfr}:=L_\cA\times L_\bfr.\]
Here we extend the Poisson structures on $L_\cX$ and $L_\cA$ to $L_{\cX,\bfr}$ and $L_{\cA,\bfr}$ so that functions on $L_\bfr$ are Casimirs. 
Then the map $\rho:L_\cA\to L_\cX$ extends to a Poisson map $L_{\cA,\bfr}\to L_{\cX,\bfr}$ which we still denote by $\rho$.
The $k$-th coordinate of a point in $L_\bfr$ determines a degree $r_k$ monic polynomial $F_k(z)\in\RR_+[z]$ with $F_k(0)=1$.

Given a choice of sign $\varepsilon\in\{\pm1\}$, define Hamiltonian functions $H^{k,\varepsilon}_\cX\in\cO_{L_{\cX,\bfr}}$ and $H^{k,\varepsilon}_\cA\in\cO_{L_{\cA,\bfr}}$ for $1\le k\le n$ by
\begin{equation}
  \label{eq:hamiltonians}
  H_\cX^{k,\varepsilon}:=-\frac{\varepsilon}{d_k}\int_0^{y_k^\varepsilon} \frac{\log\big(F_k(u)\big)}{u}du\qquad\text{and}\qquad H_\cA^{k,\varepsilon}:=-\frac{\varepsilon}{d_k}\int_0^{\hat y_k^\varepsilon} \frac{\log\big(F_k(u)\big)}{u}du.
\end{equation}
Observe that when $r_k=1$, i.e.\ $F_k(z)=1+z$, we have $H_\cX^{k,\varepsilon}:=\frac{\varepsilon}{d_k}\Li_2(-y_k^\varepsilon)$ and $H_\cA^{k,\varepsilon}:=\frac{\varepsilon}{d_k}\Li_2(-\hat y_k^\varepsilon)$.

Clearly, there is an equality $\rho^*(H_\cX^{k,\varepsilon})=H_\cA^{k,\varepsilon}$.
Write $X_\cX^{k,\varepsilon}\in\cT_{L_{\cX,\bfr}}$ and $X_\cA^{k,\varepsilon}\in\cT_{L_{\cA,\bfr}}$ for the Hamiltonian vector fields associated to $H_\cX^{k,\varepsilon}$ and $H_\cA^{k,\varepsilon}$ respectively, i.e.\ the vector fields naturally associated to the derivations $\{H_\cX^{k,\varepsilon},\cdot\}_\cX$ and $\{H_\cA^{k,\varepsilon},\cdot\}_\cA$.
\begin{lemma}
  \label{le:hamiltonian dynamics}
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, the vector fields $X_\cX^{k,\varepsilon}$ and $X_\cA^{k,\varepsilon}$ determine the following dynamics on $L_{\cX,\bfr}$ and $L_{\cA,\bfr}$:
  \begin{align}
    \label{eq:X dynamics}
    \dot y_\ell&:=\{H_\cX^{k,\varepsilon},y_\ell\}_\cX=-B_{k\ell}\log\big(F_k(y_k^\varepsilon)\big)y_\ell\quad\text{for}\quad 1\le\ell\le n;\\
    \label{eq:A dynamics}
    \dot x_j&:=\{H_\cA^{k,\varepsilon},x_j\}_\cA=-\delta_{jk}\log\big(F_k(\hat y_k^\varepsilon)\big)x_k\quad\text{for}\quad 1\le j\le m.
  \end{align}
...
\begin{proof}
  ...
  \[\{H_\cA^{k,\varepsilon},x_j\}_\cA=-\frac{\log\big(F_k(\hat y_k^\varepsilon)\big)}{d_k\hat y_k}\{\hat y_k,x_j\}=-\delta_{jk}\log\big(F_k(\hat y_k^\varepsilon)\big)x_j.\]
\end{proof}

It immediately follows that $y_k$ is a conserved quantity under the flow of $L_{\cX,\bfr}$ by the vector field $X_\cX^{k,\varepsilon}$.
Since the map $\rho$ is Poisson with $\hat y_k=\rho^*(y_k)$, we also see that $\hat y_k$ is a conserved quantity under the flow of $L_{\cA,\bfr}$ by the vector field $X_\cA^{k,\varepsilon}$.
\begin{corollary}
  \label{cor:time-one flows}
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, the time-$t$ flow $\varphi_\cX^t:L_{\cX,\bfr}\to L_{\cX,\bfr}$ of the Hamiltonian vector field $X_\cX^{k,\varepsilon}$, given on coordiates by
  \[(\varphi_\cX^t)^*(y_\ell)=\big(F_k(y_k^\varepsilon)\big)^{-tB_{k\ell}}y_\ell,\]
  and the time-$t$ flow $\varphi_\cA^t:L_{\cA,\bfr}\to L_{\cA,\bfr}$ of the Hamiltonian vector field $X_\cA^{k,\varepsilon}$, given on coordiates by
  \[(\varphi_\cA^t)^*(x_j)=\big(F_k(\hat y_k^\varepsilon)\big)^{-t\delta_{jk}}x_j,\]
...
For $1\le k\le n$ and a sign $\varepsilon\in\{\pm1\}$, define the \emph{mutation of the pair $(\tilde B,\Omega)$ in direction $k$} by $\mu_{k,\varepsilon}(\tilde B,\Omega)=\big(\mu_{k,\varepsilon}\tilde B,\Omega'\big)$, where
\begin{itemize}
...
\end{itemize}
It is an easy exercise to check that mutation of compatible pairs is an involution, more precisely $\mu_{k,\varepsilon}\mu_{k,\varepsilon'}(\tilde B,\Omega)=(\tilde B,\Omega)$ for any signs $\varepsilon,\varepsilon'\in\{\pm1\}$.
...
\begin{lemma}
  ...
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, there are Poisson maps $\tau_\cX^{k,\varepsilon}:L_{\cX,\bfr}\to L'_{\cX,\bfr}$ and $\tau_\cA^{k,\varepsilon}:L_{\cA,\bfr}\to L'_{\cA,\bfr}$ which are the identity map on the $L_\bfr$ components and are given on coordinates of $L_\cX$ and $L_\cA$ by
  \begin{align}
...
Since the mutation of $D$-compatible pairs is an involution, we obtain analogous Poisson maps from $L'_{\cX,\bfr}$ to $L_{\cX,\bfr}$ and from $L'_{\cA,\bfr}$ to $L_{\cA,\bfr}$ which, by a slight abuse of notation, we denote by the same symbols $\tau_\cX^{k,\varepsilon}$ and $\tau_\cA^{k,\varepsilon}$ respectively.
It is important to note that the maps $\tau_\cX^{k,\varepsilon}$ and $\tau_\cA^{k,\varepsilon}$ are not independent of the sign $\varepsilon$ and, moreover, that they are not involutions, i.e.\ $(\tau_\cX^{k,\varepsilon})^2$ and $(\tau_\cA^{k,\varepsilon})^2$ do not give identity maps on $L_{\cX,\bfr}$ nor on $L_{\cA,\bfr}$.
...
\[\mu_\cX^{k,\varepsilon}:=\tau_\cX^{k,\varepsilon}\circ\varphi_\cX^1:L_{\cX,\bfr}\to L'_{\cX,\bfr}\qquad\text{ and }\qquad\mu_\cA^{k,\varepsilon}:=\tau_\cA^{k,\varepsilon}\circ\varphi_\cA^1:L_{\cA,\bfr}\to L'_{\cA,\bfr}.\]
\begin{lemma}
  \label{le:cluster mutation}
  For $1\le k\le n$ and $\varepsilon\in\{\pm1\}$, the cluster mutations provide Poisson morphisms $\mu_\cX^{k,\varepsilon}:L_{\cX,\bfr}\to L'_{\cX,\bfr}$ and $\mu_\cA^{k,\varepsilon}:L_{\cA,\bfr}\to L'_{\cA,\bfr}$ which are the identity map on the $L_\bfr$ components and are given on coordinates of $L_\cX$ and $L_\cA$ by
  \begin{align}
    \label{eq:X mutation}
    (\mu_\cX^{k,\varepsilon})^*(y'_\ell)&=\begin{cases} y_k^{-1} & \text{if $\ell=k$;}\\ y_\ell y_k^{[\varepsilon r_kB_{k\ell}]_+}F_k(y_k^\varepsilon)^{-B_{k\ell}} & \text{if $\ell\ne k$;}\end{cases}\\
    \label{eq:A mutation}
    (\mu_\cA^{k,\varepsilon})^*(x'_j)&=\begin{cases} x_k^{-1}\left(\prod\limits_{i=1}^m x_i^{[-\varepsilon B_{ik}r_k]_+}\right)F_k(\hat y_k^\varepsilon) & \text{if $j=k$;}\\ x_j & \text{if $j\ne k$.}\end{cases}
  \end{align}
  Moreover, the cluster mutations $\mu_\cX^{k,\varepsilon}\mu_\cX^{k,-\varepsilon}$ and $\mu_\cA^{k,\varepsilon}\mu_\cA^{k,-\varepsilon}$ are identity maps on $L_{\cX,\bfr}$ and on $L_{\cA,\bfr}$ respectively for any choice of sign $\varepsilon$.
\end{lemma}
\begin{proof}
...
\begin{remark}
  While we have assumed that $L_\cX=\RR_+^n$, $L_\cA=\RR_+^m$, and $F_k(z)\in\RR_+[z]$ to motivate the Hamiltonian natures of $\mu_\cX^{k,\varepsilon}$ and~$\mu_\cA^{k,\varepsilon}$, the cluster mutations \eqref{eq:X mutation} and \eqref{eq:A mutation} are also well-defined when $L_\cX=(\CC^\times)^n$, $L_\cA=(\CC^\times)^m$, and $F_k(z)\in\CC[z]$ is a monic polynomial with $F_k(0)=1$.
  ...
  This holds equally well for the \emph{automorphism parts} $\varphi_\cX^1$, $\varphi_\cA^1$ and the \emph{tropical parts} $\tau_\cX^{k,\varepsilon}$, $\tau_\cA^{k,\varepsilon}$ of the cluster mutations.
  The various Poisson maps are summarized in the following commutative diagram.
  \begin{equation} 
    \label{eq:PoisCD}
    \xymatrix{
      L_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\varphi_\cA} \ar@/^1pc/[rr]|-{\mu_\cA} & L_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\tau_\cA} & L'_{\cA,\bfr} \ar[d]^{\rho} \\
      L_{\cX,\bfr} \ar[r]^{\varphi_\cX} \ar@/_1pc/[rr]|-{\mu_\cX} & L_{\cX,\bfr} \ar[r]^{\tau_\cX} & L'_{\cX,\bfr}
    }
  \end{equation}
\end{remark}
...
Fix an initial $m\times n$ exchange matrix $\tilde B^{t_0}$ and assign exchange matrices $\tilde B^t$ to the vertices $t\in\TT_n$ so that $\tilde B^{t'}=\mu_k\tilde B^t$ whenever $t$ and $t'$ are joined by an edge labeled by $k$.
...
Given an $m\times m$ matrix $\Omega^{t_0}$ compatible with $\tilde B^{t_0}$, we define matrices $\Omega^t$ compatible with $\tilde B^t$ by iterating mutations as above.

Thus, associated to each $t\in \TT_n$, we obtain a Poisson space $L_{\cX;t}$ isomorphic to either $\RR_+^n$ or $(\CC^\times)^n$ and a Poisson space $L_{\cA;t}$ isomorphic to either $\RR_+^m$ or $(\CC^\times)^m$.
...
\end{equation}
As above, we also introduce the Poisson spaces $L_{\cX,\bfr;t}$ isomorphic to either $\RR_+^n\times\prod_{k=1}^n \RR^{r_k-1}$ or to $(\CC^\times)^n\times\prod_{k=1}^n \CC^{r_k-1}$ and $L_{\cA,\bfr;t}$ isomorphic to either $\RR_+^m\times\prod_{k=1}^n \RR^{r_k-1}$ or to $(\CC^\times)^m\times\prod_{k=1}^n \CC^{r_k-1}$.

As observed above, the inverse of a cluster mutation requires a change of signs $\varepsilon$.
In general, it will important to make particular choices of the signs $\varepsilon$ as we perform sequences of mutations.
...
\begin{example}
  ...
  \begin{itemize}
    \item each column of $C^t$, known as a $\bfc$-vector, has either all non-negative entries or all non-positive entries \cite{FZ07,nakanishi-zelevinsky,gross-hacking-keel-kontsevich}.
  \end{itemize}
  Thus each exchange matrix $\tilde B^t$ mutation equivalent to $\tilde B^{t_0}$ admits a collection of \emph{tropical signs} $\varepsilon_{k;t}\in\{\pm1\}$, where $\varepsilon_{k;t}=1$ if the entries in the $k$-th column of $C^t$ are non-negative and $\varepsilon_{k;t}=-1$ if the entries in the $k$-th column of $C^t$ are non-positive. 
  ...
  Then for each vertex $t\in\TT_n$, we associate the same tropical signs $\varepsilon_{k;t}$ to the columns of $\tilde B^t$.
\end{example}

\begin{definition}
  Fix an initial $m\times n$ exchange matrix $\tilde B^{t_0}$ and an $m\times m$ matrix $\Omega^{t_0}$ which is compatible with $\tilde B^{t_0}$.
  Let $\bfr=(r_1,\ldots,r_n)$ be a sequence of positive integers.
  The \emph{(generalized) cluster varieties}
  $\cX_\bfr=\cX_\bfr(\tilde B^{t_0})$ and $\cA_\bfr=\cA_\bfr(\tilde B^{t_0})$ are obtained by gluing the cluster charts $L_{\cX,\bfr;t}$ and $L_{\cA,\bfr;t}$ respectively along the cluster mutations $\mu_\cX^{k,\varepsilon_{k;t}}$ and $\mu_\cA^{k,\varepsilon_{k;t}}$, that is
  \begin{equation}
    \label{eq:cluster varieties}
    \cX=\bigcup_{t\in\TT_n} L_{\cX,\bfr;t}\qquad\text{ and }\qquad\cA=\bigcup_{t\in\TT_n} L_{\cA,\bfr;t}.
  \end{equation}
\end{definition}
\begin{remark}
  When $r_k=1$ for all $k$, the construction above gives rise to the ordinary cluster varieties \cite{FZ01,FG09a}.
\end{remark}
...
\begin{theorem}
  \cite{FZ02,GSV16}
  Each cluster coordinate $x_{i;t}$ on the cluster chart $L_{\cA,\bfr;t}$ determines a global function on $\cA$ which is given by a Laurent polynomial in the coordinates of any other cluster chart $L_{\cA,\bfr;t'}$.
\end{theorem}
...
\end{itemize}
As above, for a sequence $\bfr=(r_1,\ldots,r_n)$ of positive integers, we introduce 
\[\cG_{\cX,\bfr}\cong\RR^n\times L_{\cX,\bfr},\qquad\cD_{\cX,\bfr}\cong\RR_+^n\times L_{\cX,\bfr},\qquad\cG_{\cA,\bfr}\cong\RR^m\times L_{\cA,\bfr},\qquad\cD_{\cA\bfr}\cong\RR_+^m\times L_{\cA,\bfr}.\]
...
Indeed, when $L_\cX=(\CC^\times)^n$ and $L_\cA=(\CC^\times)^m$, the symplectic double $\cD_\cA$ is not source-simply-connected and the Poisson map $\rho$ does not lift to a well-defined groupoid map $\bar\rho$. This is remedied in \cite{FG09c} by requiring $m=n$ and using the coordiates $B_j = s_j^{d_\ell^{-1}}$.
\end{remark}
