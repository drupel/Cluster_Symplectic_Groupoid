...
%In this case, we call $(\tilde B,\Omega)$ a \emph{Poisson pair}, this terminology is motivated by Definition~\ref{def:compatible Poisson} below.
...
As shown in Proposition~\ref{prop:groupoid covering}, the two constructions are related by a groupoid covering map from the source-simply-connected symplectic groupoid to the symplectic double.
...
\begin{definition}
  ...
  \begin{enumerate}
    \item a Poisson bracket
      ...
      \[\{fg, h\} = f\{g,h\} + g\{f,h\};\]
    \item a Poisson bivector $\pi \in \cT^2_M$ such that $[\pi, \pi] = 0$, where $[\cdot, \cdot]$ denotes the Schouten-Nijenhuis bracket of multivector fields.
  \end{enumerate}	
  The pair $(M, \pi)$, or equivalently $(M, \{,\})$, is called a \emph{Poisson manifold}.
\end{definition}
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item an associative multiplication map $m: \cG {_\beta \times_\alpha} \cG \to \cG, \enskip (g, h) \mapsto gh$ whenever $\beta(g)=\alpha(h)$;\footnote{so our groupoids will always act on the right?}
  ...
  \end{enumerate}
  A Lie groupoid $\cG \rightrightarrows M$ is \emph{source-connected} if $\alpha^{-1}(x)$ is connected for every $x \in M$ and is \emph{source-simply-connected} if, in addition, $\alpha^{-1}(x)$ simply-connected for every $x \in M$.
\end{definition}

\begin{definition}
  A \emph{symplectic groupoid} is a Lie groupoid structure $\cG \rra M$ with a symplectic structure $\omega \in \Omega^2(\cG)$ such that the graph of the multiplication map $\Gamma_m = \{(g, h, gh) \in \cG \times \cG \times \cG\}$ is a Lagrangian submanifold of $(\cG \times \cG \times \cG, \omega \oplus \omega \oplus -\omega)$.
\end{definition}
...
That is, the source $\alpha$ is Poisson and the target $\beta$ is anti-Poisson.
...
The integrability of Poisson manifolds, and more generally the integrability of Lie algebroids, was characterized in \cite{MR1973056, MR2128714}.

Some notable examples of symplectic groupoids include: the Drinfeld double of Poisson Lie groups \cite{MR1054741}, the double Bruhat cells \cite{LuM16}, the blow-up groupoid of log symplectic manifolds \cite{MR3214314}, and the symplectic double of cluster $\cX$-variety \cite{MR2470108}.

We recall the construction of local symplectic groupoids by Poisson sprays \cite{MR2900786, CMS17}.
\begin{definition}
  For a Poisson manifold $(M, \pi)$, a Poisson spray is a vector field $X \in \cT_{T^*M}$ such that
  \begin{enumerate}
    \item for $(\bfp,\bfx) \in T^*M$,
      \[\tau_* X|_{(\bfp,\bfx)} = \pi^\flat(\bfp),\]
      where $\tau: T^*M \to M$ is the bundle projection;
    \item $X$ is homogeneous of degree 1, i.e.
      \[(m_\lambda)_*(X) = \lambda X,\] 
      where $m_\lambda: T^*M \to T^*M, \enskip (\bfp,\bfx) \mapsto (\lambda\bfp,\bfx)$ is the fiberwise scaling map.
  \end{enumerate}
\end{definition}

\begin{theorem} \cite{MR2900786, CMS17} 
  \label{thm:poissp}
  For a smooth Poisson manifold $(M, \pi)$ with a Poisson spray $X \in \cT_{T^*M}$, a neighborhood $U$ of the zero section of $T^*X$ is a local symplectic groupoid over $(M, \pi)$ with the following structures:
  \begin{enumerate}
...
\begin{remark}
  ...
  In the situation Theorem~\ref{thm:poissp}, the time-$1$-flow of the Poisson spray $X$ may be outside $U$.
  In general, it is not possible to extend the local symplectic groupoid structure to the entire cotangent bundle $T^*M$: the Poisson spray $X$ may not be complete; the flow of the Poisson spray $X$ may loop; or the 2-form $\overline{\omega}$, though non-degenerate near the zero section of $T^*M$, may be degenerate outside $U$.
  However, in nice cases these pathologies do not occur and the local symplectic groupoid structure maps can be used as motivation for defining a global groupoid structure.
\end{remark}

Let $L$ be either $\RR^m$ or $\CC^m$ with coordinates $\bfx = (x_1, \ldots, x_n)$ and let $\Omega = (\Omega_{ij})$ be an $m\times m$ skew-symmetric matrix.
\begin{lemma}
  ...
  the vector field $X \in \cT_{T^*L}$ given by
  \begin{equation} 
    \label{eq: PoisSp}
...
In the case when $L = \RR^m$, this Poisson spray $X$ gives rise to the symplectic groupoid structure in the following Theorem~\ref{thm:PoiSpLogC}.
Note that the averaging technique used in Theroem~\ref{thm:poissp} to obtain the symplectic form works only when the underlying field is $\RR$.
However, the statements in Theorem~\ref{thm:PoiSpLogC} can easily be checked directly when the underlying field is $\CC$.
\begin{theorem} 
  \label{thm:PoiSpLogC}
  Let $L$ be either $\RR^m$ or $\CC^m$.
  For the log-canonical Poisson structure 
  \[\pi = \sum_{i > j} \Omega_{ij} x_i x_j\frac{\partial}{\partial x_i} \wedge \frac{\partial}{\partial x_j}\]
  on $L$, there is a source-simply-connected symplectic groupoid $(\cG_L, \omega) \rra (L, \pi)$ with the following structures:
  \begin{enumerate}
...
%				& (\bfp, \bfx) \mapsto \left(-e^{-\sum\limits_{i} \Omega_{1i} x_ip_i} p_1, \ldots, -e^{-\sum\limits_{i} \Omega_{mi} x_ip_i} p_m, e^{\sum\limits_{i} \Omega_{1i} x_ip_i} x_1, \ldots, e^{\sum\limits_{i} \Omega_{mi} x_ip_i} x_m\right);
	...
	\[\begin{aligned}
	    ...
	    & \big((\bfp, \bfx), (\bfp', \bfa \circ \bfx)\big) \mapsto (\bfa \circ \bfp' + \bfp, \bfx );
...
%				& \mapsto \left(e^{\sum\limits_{i} \Omega_{1i} x_ip_i} p'_1 + p_1, \ldots, e^{\sum\limits_{i} \Omega_{mi} x_ip_i} p'_m + p_m, \bfx \right);
	...
	\[\omega = \sum_{i} dp_i \wedge dx_i
	    - \left(\sum_{i, j} \Omega_{ij}x_i p_j d p_i \wedge d x_j + \sum_{j < i} \Omega_{ij}p_ip_j d x_i \wedge d x_j + \sum_{j < i} \Omega_{ij}x_ix_j d p_i \wedge d p_j\right).\]
  \end{enumerate}
\end{theorem}

\begin{proof}
  In the case when the underlying field is $\RR$, the flow $\varphi_X^t: T^*L \to T^*L$ of the Poisson spray \eqref{eq: PoisSp} can be computed as follows:
...
%		\varphi_X^t: (\bfp, \bfx) \mapsto \left(e^{-t\sum\limits_{i} \Omega_{1i} x_ip_i} p_1, \ldots, e^{-t\sum\limits_{i} \Omega_{mi} x_ip_i} p_m, e^{t\sum\limits_{i} \Omega_{1i} x_ip_i} x_1, \ldots, e^{t\sum\limits_{i} \Omega_{mi} x_ip_i} x_m\right)
  The flow exists for all time $t \in \RR$ and has no loops.
  Moreover, $\omega$ is non-degenerate since
  \[\omega^m = m \bigwedge\limits_{1\leq i\leq m} dp_i \wedge dx_i\]
  is a volume form.
  It is straightforward to check that the symplectic groupoid structure above is induced from the Poisson spray $X$ in \eqref{eq: PoisSp} following Theorem~\ref{thm:poissp}.

  In the case when the underlying field is $\CC$, one can easily verify the symplectic groupoid axioms directly.
\end{proof}

If we restrict $L$ to be either $\RR_+^m$ or $(\CC^\times)^m$ where $\RR_+ = (0, \infty)$ and $\CC^\times = \CC \setminus \{0\}$, then there is the source-connected symplectic groupoid of the log canonical Poisson space $(L, \pi)$, called the \emph{symplectic double} \cite{MR2470108}.
\begin{theorem} 
  \cite{MR2470108}
...
Similar to the Lie groups, for an integrable Lie algebroid, there is a groupoid morphism from the source-simply-connected groupoid to any other integration.
With the chosen notation, we have a simple expression for the groupoid morphism $\kappa: \cG_L \to \cD_L$.
\begin{proposition}
  \label{prop:groupoid covering}
  The covering map
