...
For a symplectic groupoid $(\cG, \omega) \rra \cX$, there is a natural Poisson structure $\pi$ on $\cX$ such that $\alpha: (\cG, \omega) \to (\cX, \pi)$ and $\beta: (\cG, \omega) \to (\cX, -\pi)$ are Poisson maps. That is, the source $\alpha$ is Poisson and the target $\beta$ is anti-Poisson. Poisson manifolds that arise this way are called integrable. As a Lie groupoid, $\cG \rra \cX$ integrates the Poisson Lie algebroid $T^*\cX$ with the anchor map $\pi^\flat: \cT^*_\cX \to \cT_\cX$ \cite{MR866024}; and as a Poisson groupoid, $(\cG, \omega) \rra (\cX, \pi)$ integrates the Lie bialgebroid $(\cT^*\cX, \cT\cX)$ where the cotangent bundle $\cT^*\cX$ carries Poisson Lie algebroid structure and the tangent bundle $\cT\cX$ carries the standard the Lie algebroid structure \cite{MR1262213}. The integrability of Poisson manifolds, and more generally the integrability of Lie algebroids, were characterized in \cite{MR1973056, MR2128714}. Here are a few notable examples of symplectic groupoids: the Drinfeld double of Poisson Lie groups \cite{MR1054741}, the blow-up groupoid of log symplectic manifolds \cite{MR3214314} and the symplectic double of cluster $\cX$-variety \cite{MR2470108}. We recall the construction of local symplectic groupoids in\cite{MR2900786, CMS17} by Poisson sprays.

\begin{definition}
For a Poisson manifold $(\cX, \pi)$, a Poisson spray is a vector field $X \in \cT_{\cT^*\cX}$ such that
	\begin{enumerate}
		\item for $(x,p) \in \cT^*\cX$,
			$$
				\tau_* X|_{(x,p)} = \pi^\flat(p)
			$$
			where $\tau: \cT^*\cX \to \cX$ is the bundle projection;
		\item
			$X$ is homogeneous of degree 1, i.e.
			$$
				(m_\lambda)_*(X) = \lambda X 
			$$
			where $m_\lambda: \cT^*\cX \to \cT^*\cX, \enskip (x,p) \mapsto (x,\lambda p)$ is the fiberwise scaling map.
	\end{enumerate}
\end{definition}

\begin{theorem} \cite{MR2900786, CMS17} \label{thm:poissp}
For a smooth Poisson manifold $(\cX, \pi)$ with a Poisson spray $X \in \cT_{\cT^*\cX}$, a neighbourhood $\cU$ of the zero section of $\cT^*\cX$ is a local symplectic groupoid over $(\cX, \pi)$ with the following structures:
	\begin{enumerate}
		...
		\item the target map is
			...
			$$
		where $\varphi_X^1: \cT^*\cX \to \cT^*\cX$ is the time-$1$-flow of $X$;
...
In general, the local symplectic groupoid structure cannot be extended to the total space $\cT^*\cX$. The Poisson spray $X$ may not be complete; the flow of the Poisson spray $X$ may loop; and the 2-form $\overline{\omega}$, though non-degenerate near the zero section of $\cT^*\cX$, may be degenerate in general.
\end{remark}

Let $\cX$ be either $\RR^m$ or $\CC^m$ and $\{(x_1, \ldots, x_n\}$ be coordinates on $\cX$, and let $\Omega_{ij}, 1 \leq i,j \leq m$ be a skew-symmetric matrix. For the log-canonical Poisson structure $\{x_i, x_j\} = \Omega_{ij} x_i x_j$, or equivalently $\sum_{i > j} \Omega_{ij} x_ix_j\frac{\partial}{\partial x_i} \wedge \frac{\partial}{\partial x_j}$, the vector field $X \in \cT_{\cT^*\cX}$:
...
$$
there is a source-simply-connected symplectic groupoid $(\cG_\cX, \omega) \rra (\cX, \pi)$ with the following structures:
	\begin{enumerate}
		\item $\cG_\cX \cong \cT^*\cX$;
...
There is another interesting symplectic groupoid of the log canonical Poisson space $(\cX, \pi)$, the symplectic double in \cite{MR2470108}. We denote this symplectic groupoid by $\cD_\cX \rra \cX$.

{\color{red} Introduce the symplectic double from \cite{MR2470108} in local charts.}

The symplectic groupoid $\cG_\cX \rra \cX$, being source-simply-connected, is the covering of $\cD_\cX \rra \cX$ such that each source fiber of $\cG_\cX \rra \cX$ is the universal cover of the corresponding source fiber of $\cD_\cX \rra \cX$.


{\color{red} Give the explicit covering map $\cG \to \cD$.}
