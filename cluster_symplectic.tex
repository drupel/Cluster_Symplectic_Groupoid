...
The groupoid $\cG \rra L$ is the source-simply-connected symplectic groupoid. \footnote{Corrected! My original statement creates confusion.}
...
We also lift the cluster ensemble map $\rho: L^+_\cA \to L^+_\cX$.
When the exchange matrix is square, the Poisson map $\rho$ can be lifed to groupoid maps for all three groupoids $\cG$, $\cD$ and $\cB$.
However when the exchange matrix is not sqaure, $\rho$ does not lift to bona fide groupoid maps. This is because Poisson maps in general lift Lagrangian relations of symplectic groupoids.
\bigskip
