...
\end{equation}
Since $A$ is a subbundle of $T\cG|_{\mathtt{1}_M}$,  a section of $A$ can be thought as a section of $T\cG$ supported on $\mathtt{1}_M$, which can be left translated along the source fibers of  $\cG \rightrightarrows M$ to yield a left-invariant vector field on $\cG$. Similar to Lie groups, the left-invariant vector fields of a Lie groupoid is closed under the Lie bracket of vector fields, and so we have a Lie bracket on $\Gamma(M, A)$.
