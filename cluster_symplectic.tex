...
We have presented thus far the Hamiltonian viewpoint of mutation for cluster charts. Our goal now is to lift these results to the level of the symplectic groupoids $\cG_\cX$, $\cB_\cX$, $\cD_\cX$ and $\cG_\cA$, $\cB_\cA$, $\cD_\cA$ integrating the log-canonical Poisson structures on $L^+_\cX\cong\RR_+^n$ and $L^+_\cA\cong\RR_+^m$ respectively as in Section~\ref{sec:local}. In general,
 a Poisson map does not induce Lie algebroid morphisms and hence do not integrate to bona fide symplectic groupoid maps. Instead a Poisson map induces a Lie algebroid comorphism. Following \cite{MR3089758}, we outline the procedure of lifting Lie algebroid morphisms and comorphisms to Lie groupoid morphisms and comorphisms.
\begin{definition} \cite{Higgins1990, MR2157566, MR3089758}
 	...
 	\begin{enumerate}
 		\item A Lie algebroid morphism is a bundle map $(\Phi, \varphi): (A_1, M_1) \to (A_2, M_2)$ such that the pullback of sections $\Phi^*: \Gamma(\wedge^\bullet A_2) \to \Gamma(\wedge^\bullet A_2)$ is a chain map of complex .
 		\item A Lie algebroid comorphism is a bundle comorphism $(\varphi, \Phi)$ with
 			\[ \varphi: M_1 \to M_2,~ \Phi: \varphi^! A_2 = M_1 {\times_{M_2}} A_2 \to A_1 \]
 			such that if $\varphi_* \circ \rho_1 \circ \Phi = \rho_2$. 
 	\end{enumerate}
\end{definition}
For a diffeomorphism $\varphi: M_1 \to M_2$, then $(\varphi, \Phi)$ is a Lie algebroid morphism if and only if $(\varphi^{-1}, \Phi^*)$ is a Lie algebroid comorphism.
\begin{definition} \cite{MR2157566, MR3089758} \label{def:algcom}
	Let $\cG_1 \rra M_1$ and $\cG_2 \rra M_2$ be Lie groupoids.
	\begin{enumerate}
 		\item A Lie groupoid morphism from $\cG_1$ to $\cG_2$ is a map $\Phi: \cG_1 \to \cG_2$ that is compatible with groupoid structures. 
 		\item A Lie groupoid comorphism from $\cG_1$ to $\cG_2$ is a base map $\varphi: M_1 \to M_2$ together with a map
 			\[ \Phi: M_1 {_\varphi \times_\alpha} \cG_2 \to \cG_2 \]
 		that is compatible with groupoid structures. (For details, see p.5 of \cite{MR3089758}.) 
 	\end{enumerate}
\end{definition}
Note the difference in convention: the source and target maps in Definition~\ref{def:algcom} are switched comparing to the convention in \cite{MR3089758}. 

In general, a Lie algebroid morphism from $A_1$ to $A_2$ integrates to a Lie groupoid morphism from $\cG_1$ to $\cG_2$ if $\cG_1$ is source-simply-connected; a Lie algebroid comorphism from $A_1$ to $A_2$ integrates to a Lie groupoid comorphism from $\cG_1$ to $\cG_2$ if $\cG_2$ is source-simply-connected and the algebroid comorphism is complete. Concretely, a morphism or a comorphism from $A_1$ to $A_2$ is a Lie subalgebroid of $A_1 \times A_2$, and the exponential map $A_1 \times A_2 \to \cG_1 \times \cG_2$ integrates to a Lie subgroupoid of $\cG_1 \times \cG_2$ which is the graph of a morphism or a comorphism from $\cG_1$ to $\cG_2$.

The Poisson map $\varphi: (M_1, \pi_1) \to (M_2, \pi_2)$ induces the pullback $\varphi^*: \varphi^!  T^*_{\pi_2} M_2 \to T^*_{\pi_1} M_1$, which means that the following diagram commutes \cite{Higgins1990}.
\begin{equation} 
	\xymatrix{
		T^* M_1 \ar[d]^{\pi_1^\flat} & \varphi^! T^*_{\pi_2}  M_2 \ar[l]_{\varphi^*} \ar[r]& T^* M_2  \ar[d]^{\pi_1^\flat} \\
		T M_1 \ar[rr]^{\varphi_*} & & TM_2
	}
\end{equation}
The comorphism induced by the Poisson map $\varphi: (M_1, \pi_1) \to (M_2, \pi_2)$ is complete if for a complete Hamiltonian vector field $X_f$ with $f \in \cO_{M_2}$, the Hamiltonian vector field $X_{\varphi^*f}$ is also complete. That is, a Poisson map $\varphi: (M_1, \pi_1) \to (M_2, \pi_2)$ lifts to a symplectic groupoid comorphism from $(\cG_1, \omega_1)$ to $(\cG_2, \omega_2)$ if $\cG_2$ is source-simply-connected and the Poisson map $\varphi$ is complete. In fact, the graph of $\varphi$ which is a coisotropic submanifold of $(M_1 \times M_2, \pi_1 \oplus -\pi_2)$ integrates to a Lagrangian subgroupoid of $(\cG_1 \times \cG_2, \omega_1 \oplus -\omega_2)$ \cite{MR2063018}.

If the Poisson map $\varphi: (M, \pi) \to (M, \pi)$ is a diffeomorphism, then $\varphi$ induces bother a Lie algebroid morphism and a Lie algebroid comorphism, which can be lifted to either symplectic groupoid morphism or a symplectic groupoid comorphis. Following \cite{MR2504211}, we outline a method to lift the Hamiltonian Poisson maps to Hamiltonian symplectic groupoid morphisms.
...
\begin{lemma}
  The Poisson ensemble map $\rho:L_\cA\to L_\cX$ \eqref{eq:ensemble} induces comorphisms of groupoids as follows:
  \begin{equation} \label{eq:gpdensemble}
  	x_i p_i =  \sum_{k=1}^n B_{ik} y_k q_k
  \end{equation}
\end{lemma}
\begin{remark}
...
As a first step in lifting the separation of additions formulas~\eqref{eq:separation of additions 1} and~\eqref{eq:separation of additions 2} to the level of groupoids we record the simple observation that the mutations respect target maps $\beta$.
\begin{lemma}
  \label{le:target compatibility}
  This may not be necessary to write down.
\end{lemma}
