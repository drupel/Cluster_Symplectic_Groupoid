...
\begin{remark}
	...
	With the notations in Theorem~\ref{thm:SymDBLoc} and the change of variables $e^{\chi_i} = x_i$ and $e^{\xi_i} = s_i$, we have a symplectic vector space $(\cU, \omega)$ where $\cU$, with coordinates $(\xi_1, \ldots, \xi_m, \chi_1, \ldots, \chi_m)$, is either $\RR^{2m}$ or $\CC^{2m}$ and
	\[
...
For the standard symplectic structure $\omega_0 = \sum_{i} d\chi_i \wedge d\xi_i$ on $\cU$, since
	...
	\]
is supported on the source fiber, it follows that $\beta_*(\omega_0^{-1}) = \beta_*(\omega^{-1}) = -\pi$. The symplectic realization $\beta: (\cU, -\omega_0) \to (L, \pi)$ is the central object of study in \cite{MR3691969}.
