...
Observe that both $\cB \rra L$ and $\cD \rra L$ naturally receive maps from the source-simply-connected symplectic groupoid $\cG \rra L$.
...
\begin{example}
  \label{ex:running 2}
  As in Example~\ref{ex:running 1}, we take 
...
\begin{example}
  Continuing from Example~\ref{ex:running 2}, we see from Lemma~\ref{le:dual vectors} that $C_t^\vee=C_t^T$ and $G_t^\vee=G_t^T$ for $t\in\TT_n$, where $(-)^T$ denotes the transpose of a matrix.
  The iterated groupoid mutations can thus be computed from the data given in Example~\ref{ex:running 2} using the separation of additions formulas in Theorem~\ref{th:groupoid separation of additions}.
