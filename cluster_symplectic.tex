...
The groupoid $\cG \rra L$ is the source-simply-connected symplectic groupoid when the underlying field is $\CC$.
\footnote{Does something go wrong in the real case?}
...
Over an octant $L_+$ of $L$ the groupoid $\cD \rra L$ is actually symplectic and we denote its restiction by $\cD_+ \rra L_+$, this is the groupoid considered by Fock and Goncharov in the case of $\cX$-varieties.
...
We also lift the cluster ensemble map $\rho: L_\cA \to L_\cX$ to groupoid maps.
When the underlying field is $\RR$, the Poisson map $\rho$ can be lifed to all three groupoids $\cG$, $\cD$ and $\cB$.
When the underlying field is $\CC$, the Poisson map $\rho$ can only be lifted to the source-simply-connected symplectic groupoid $\cG$.
\footnote{Are we sure we cannot lift the map always?}
\bigskip
...
\begin{itemize}
	...
	$$
		L = \RR^m \text{ or } \CC^m, \quad L_+ = \RR_+^m \text{ or } \CC_\times^m, \quad \bar L_+ = \bar\RR_+^m.
	$$
...
\begin{definition}
  ...
  \begin{enumerate}
    \item $\cG$ and $M$ are smooth manifolds (or complex manifolds);
...
With our convention, $\cG$ acts on itself on the left. Next we recall the notion of a Lie algebroid, which is the infitesimal object of a Lie groupoid.
\begin{definition}
	...
	\begin{enumerate}
		...
		\item $[\cdot, \cdot]$ is a Lie bracket on the space of sections $\Gamma(M, A)$, and
		\item $\rho: A \to TM$ is a bundle morphism preserving the Lie bracket
	\end{enumerate}
	that satisfies the Leibniz rule: for $X, Y \in \Gamma(M, A)$ and $f \in \cO_M$,
	\[
...
A Poisson groupoid map from $(\cG_1, \sigma_1) \rra M_1$ to $(\cG_2, \sigma_2) \rra M_2$ is a Lie groupoid map $\varphi: \cG_1 \to \cG_2$ which is Poisson. For a Poisson groupoid $(\cG, \sigma) \rra M$, there is a natural Poisson structure $\pi$ on $M$ such that $\alpha: (\cG, \sigma) \to (M, \pi)$ and $\beta: (\cG, \sigma) \to (M, -\pi)$ are Poisson maps. For a Poisson manifold $(M, \pi)$, one could ask if there is a symplectic groupoid $(\cG, \omega) \rra (M, \pi)$ such that $\alpha: (\cG, \omega) \to (M, \pi)$ is a Poisson map. If the answer is yes, we say that the Poisson manifold $(M, \pi)$ is \emph{integrable}. The Lie algeboid of a symplectic groupoid $\cG \rra M$ is the cotangent bundle $T^*M$ \cite{MR866024} with the anchor map
$$
...
Just like the infinitesimal object of a Poisson Lie group is a Lie bialgebroid, the infinitesimal object of a Poisson groupoid is a Lie bialgebroid \cite{MR1262213}. As a Poisson groupoid, the symplectic groupoid $(\cG, \omega) \rra (M, \pi)$ integrates the Lie bialgebroid $(T^*_\pi M, TM)$.
The integrability of Poisson manifolds, and more generally the integrability of Lie algebroids, is characterized in \cite{MR1973056, MR2128714}. The source fibers of a symplectic groupoid are symplectic orthogonal to the target fibers.
...
In general, a Poisson map $\varphi: (M_1, \pi_1) \to (M_2, \pi_2)$ does not integrate to a symplectic groupoid map $\varphi: (\cG_1, \omega_1) \rra (\cG_2, \omega_2)$.
...
However as a recurring theme throughout \S\ref{sec:mutations}, the cluster mutations always lift to groupoid mutations.

We focus on the symplectic groupoids of log-canonical Poisson structures.

\begin{definition} \label{def: logPoisson}
...
\[
	\exp: T^*_\pi L \to \cG, \qquad (\bfp, \bfx) \mapsto \bfa \circ \bfx,
\]
...
Next, we describe the Lie algebroid of $\cD \rra L$. We introduce coordinates on $A_\cD \cong L\times L$ with the bundle projection
\[
...
\[
	\rho_\cD: A_\cD \cong L\times L \to TL, \qquad (\bfxi, \bfx) \mapsto \sum_{i,j}\xi_j x_i\Omega_{ji}\frac{\partial}{\partial x_i},
\]
and the kernel of $\rho_\cD$ has trivial bracket.
...
\begin{proof}
With the notations in Theorem~\ref{thm:SymDBLoc}, if we write $e^{\xi_i} = s_i$, then $\Lie \cD = \ker \left(\alpha_*: T\cD|_{\mathtt{1}_L} \to TL \right)$ is generated by $\frac{\partial}{\partial \xi_i}, i = 1, \ldots, m$. Rewriting the target map $\beta$ in $(\bfxi, \bfx)$ coordinates,
\[
	\beta: (\bfxi, \bfx) \mapsto \left(e^{\sum_i\Omega_{i1}\xi_i}x_1, \ldots, e^{\sum_i\Omega_{im}\xi_i}x_m\right).
\]
Therefore, 
\begin{equation} \label{eq: D anchor}
	\beta_* \left(\frac{\partial}{\partial \xi_j}\right) = \sum_i x_i \Omega_{ji}  \frac{\partial}{\partial x_i}, \quad 
	\beta_* \left(\sum_j \xi_j \frac{\partial}{\partial \xi_j}\right) = \sum_i \xi_j x_i \Omega_{ji}  \frac{\partial}{\partial x_i}.
\end{equation}
This shows that the bundle map
\[
	A_\cD \to \Lie \cD, \qquad (\bfxi, \bfx) \to \sum_j \xi_j \frac{\partial}{\partial \xi_j}
\]
is a Lie algebroid isomorphism.
%For a $m\times m$ skew-symmetric matrix $\Omega$, $\bfx \in L$ and $\bfs \in L_+$, we define the matrix
%\[
%	I_{\bfxi} \Omega I_\bfx = \begin{bmatrix}
%		\xi_1x_1\Omega_{11} & \xi_1x_2\Omega_{12} & \cdots & \xi_1x_m\Omega_{1m} \\
%		\xi_2x_1\Omega_{21} & \xi_2x_2\Omega_{22} & \cdots & \xi_2x_m\Omega_{2m} \\
%		\vdots & \vdots & \ddots & \vdots \\
%		\xi_mx_1\Omega_{m1} & \xi_mx_2\Omega_{m2} & \cdots & \xi_mx_m\Omega_{mm} \end{bmatrix}
%\]
\end{proof}

Fixing an $m\times m$ skew-symmetric matrix $\Omega$ and the corresponding log-canonical Poisson space as in Definition~\ref{def: logPoisson}, the cotangent Lie algebroid $T^*_\pi L$ is not isomorphic to the Lie algebroid $A_\cD$. In fact, $T^*_\pi L$ is isomorphic to an iterated elementary modification of $A_\cD$ and we obtain a source-connected symplectic groupoid of $(L,\pi)$ by the blow-up construction \cite{MR3214314}. 
...
\begin{definition}
	...
	$$
		\Gamma(M, [E\!:\!F]) = \{s \in \Gamma(M, E) : s|_L \in \Gamma(L, F) \}.
	$$
\end{definition}

\begin{proposition} \cite{MR3214314}
	If $A$ be a Lie algebroid over $M$ and let $B$ be a subbundle of $A|_L$ for some hypersurface $L$ such that $B$ is also a Lie algebroid over $L$, then $[A\!:\!B]$ is a Lie algebroid over $M$.
\end{proposition}

Let $\cG \rra M$ be a Lie groupoid and let $\cH \rra L$ be a Lie subgroupoid over a hypersurface $L$. We denote the blowup of $\cG$ along $\cH$ by $\mathrm{Bl}(\cG, \cH)$ with the blow-down map $\nu: \mathrm{Bl}(\cG, \cH) \to \cG$ and write $[\cG\!:\!\cH] = \mathrm{Bl}(\cG, \cH) \setminus (S_L \cup T_L)$ where $S_L$ is the proper transform of $\alpha^{-1}(L)$ and $T_L$ is the proper transform of $\beta^{-1}(L)$.

\begin{theorem} \cite{MR3214314}
	For a Lie groupoid $\cG \rra M$ and a Lie subgroupoid $\cH \rra L$ for some hypersurface $L$, there is a unique Lie groupoid structure $[\cG\!:\!\cH] \rra M$ such that the blow-down map $\nu: [\cG\!:\!\cH] \to \cG$ is a groupoid morphism. Moreover, blowup of Lie groupoids corresponds to elementary modification of Lie algebroids. That is,
	\[
		\Lie[\cG\!:\!\cH] = [\Lie\cG\!:\!\Lie\cH].
	\]
\end{theorem}

Our task is clear now. We need to find the Lie subalgebroids of $A_\cD$ such that the iterated elementary modifications along these Lie subalgebroids yield the cotangent algebroid $T^*_\pi L$.

For each $k = 1, \ldots, m$, we define the Lie subalgebroid $A_\cD^k$ of $A_\cD$ as follows. Let $L_k$ be the codimension 1 subpace in $L$ defined by $x_k = 0$. As a vector bundle, $A_\cD^{k}$ is the corank $1$ subbundle of $A_\cD |_{L_k}$ defined by $\xi_k = 0$. That is, the anchor map of $A_\cD^k$ is given by
\begin{align*}
	\rho_\cD^k: ~& A_\cD^k \cong L_k\times L_k \to TL_k, \\
	& (\xi_1, \ldots, \xi_{k-1}, \xi_{k+1}, \ldots, \xi_m, x_1, \ldots, x_{k-1}, x_{k+1}, \ldots, x_m) \mapsto \sum_{1\le i \le m, j\ne k}\xi_j x_i\Omega_{ji}\frac{\partial}{\partial x_i}.
\end{align*}
...
\begin{proof}
	For the Poisson structure $\pi = \Omega_{ij}x_i x_j$, we have the Poisson anchor map
	\begin{equation}	\label{eq:log-can Poiss}
		\pi^\flat: T^*L \to TL, \qquad dx_j \to \sum_i x_ix_j \Omega_{ji}  \frac{\partial}{\partial x_i}.
	\end{equation}
	The result follows by comparing \eqref{eq:log-can Poiss} with the anchor map \eqref{eq: D anchor} for $\Lie \cD$ which is isomorphic to $A_\cD$, 
\end{proof}

To obtain a source-connected symplectic groupoid of the log-canonical Poisson space $(L, \pi)$ which integrates $T^*_\pi L$, we need to iteratively blow up the Poisson groupoid $\cD \rra L$ along the subgroupoids integrating $A_\cD^k$, for $k =1, \ldots, m$. Indeed the Lie subalgebroid $A_\cD^k$ integrates to the subgroupoids $\cD_k \rra L_k$ defined by $x_k=0$ and $s_k=1$, so we define the Lie groupoid
\[
	\cB = [\ldots[[\cD\!:\!\cD_1]\!:\!\cD_2] \ldots \!:\!\cD_m]
\]
Using the blowup coordinates $u_i = \frac{s_i-1}{x_i}$, we have the following symplectic groupoid structure on $\cB \rra L$.
...
\begin{theorem} 
  ...
  \begin{enumerate}
    ...
    \item the inverse map is
      \[\iota: \cB \to \cB, \qquad (\bfu, \bfx) \mapsto \left(-\frac{u_1}{u_1x_1+1}, \ldots, -\frac{u_m}{u_mx_m+1}, x_1 \prod_{i=1}^m (u_ix_i+1)^{\Omega_{i1}}, \ldots, x_m \prod_{i=1}^m (u_ix_i+1)^{\Omega_{im}}\right);\]
    \item the multiplication map is
      \[\begin{aligned}
...
\begin{proof}
	These are the same groupoid structures as in Theorem~\ref{thm:SymDBLoc} with the change of variables $u_i = \frac{s_i-1}{x_i}$. The difference is precisely that $\omega_\cB$ is no longer singular. Note that $\omega_\cB$ is also non-degenerate since
...
\begin{remark}
Recall that the infinitesimal object of a Poisson groupoid is a Lie bialgebroid \cite{MR1262213}. The Lie bialgebroid of the Poisson groupoid $\cD \rra L$ is isomorphic to $(A_\cD, TL(-\log D))$ where $TL(-\log D)$ is the log tangent bundle with respect to the normal crossing divisor $D = L_1 + L_2 + \ldots + L_k$ \cite{MR3214314}. That is
\[
	TL(-\log D) = [\ldots[[TL\!:\!TL_1]\!:\!TL_2] \ldots \!:\!TL_m],
\]
or equivalently the sections of $TL(-\log D)$ are the vector fields on $L$ tangent to $L_k$ for $k = 1, \ldots, m$. As a vector bundle $TL(-\log D)$ is generated by $x_k \frac{\partial}{\partial x_k}, k = 1, \ldots, m$, and its dual bundle $T^*L(-\log D)$ is generated by $\frac{d x_k}{x_k}, k = 1, \ldots, m$. The bundle map
\[
	A_\cD \to T^*L(-\log D), \qquad (\bfxi, \bfx) \mapsto \sum_k\xi_k\frac{d x_k}{x_k}
\]
identifies $A_\cD$ with $T^*L(-\log D)$.

The iterated blow-up construction that takes $\cD \rra L$ to $\cB \rra L$ corresponds to the elementary modification that takes the Lie bialgebroid $(A_\cD, TL(-\log D))$ to the Lie bialgebroid $(T^*_\pi L, TL)$ where $T^*_\pi L$ is an elementary lower modification of $A_\cD$ and $TL$ is an elementary upper modification of $TL(-\log D)$. See the first author's thesis for details \cite{MR3312911}.
\end{remark}

For an integrable Lie algebroid, every source-connected groupoid receives a surjective groupoid map from the source-simply-connected groupoid. This means that we have a natural symplectic groupoid $\kappa:\cG\to\cB$. Together with the blow-down map $\nu:\cB\to\cD$, the following commutative diagram summarizes the maps among the groupoids $\cG$, $\cB$ and $\cD$.
\begin{equation} 
    \label{eq:LGpdCD}
    \xymatrix{
...
\begin{remark}
	If $L = \RR^m$, then $\cB \rra L$ is source-simply-connected and all three maps $\kappa: \cG \to \cB$ is a groupoid isomorphism, but if $L = \CC^m$, then $\cB$ is not source-simply-connected and $\kappa: \cG \to \cB$ is a covering map for each source fiber.
\end{remark}

We summarize the pros and cons of the three groupoids $\cG \rra L$, $\cB \rra L$ and $\cD \rra L$ .

The symplectic groupoid $\cG \rra L$ is source-simply-connected when the underlying field is $\CC$, and as we will see in \S\ref{sec:groupoid mutations}, is the only one aming the three groupoids for which the Poisson ensemble map $\rho: L_\cA \to L_\cX$ from an holomorphic $\cA$-vareity to its corresponding holomorphic $\cX$-vareity can be lifted to a symplectic groupoid map $\rho: \cG_\cA \to \cG_\cX$. The disadvantage of $\cG \rra L$ is that its groupoid structure maps are transcendental.

On the other hand, the multiplicative Poisson structure of $\cD \rra L$ have a simple expression and its groupoid structure maps are algebraic, but it is only symplectic when restricted to $L_+$. Both $\cG \rra L$ and $\cB \rra L$ are symplectic groupoids over $L$, and as we will see in \S\ref{sec:groupoid mutations}, this enables us to define the groupoid mutations for $\cG$ and $\cB$ over $\bar{L}_+$ when the underlying field is $\RR$.

Comparing to $\cG \rra L$, the symplectic groupoid $\cB \rra L$ has the added plus that its groupoid structure maps are algebraic, but the tradeoff comparing to $\cD \rra L$ is that its multiplicative Poisson structure and its groupoid structures is somewhat more complicated.
...
We identify the $\ell$-th component of a point in $L_\bfr$ with the degree $r_\ell$ monic polynomial $P_\ell(z)\in\RR_+[z]$ with $P_\ell(0)=1$ given by
\[P_\ell(z)=1+a_{\ell,1}z+\cdots+a_{\ell,r_\ell-1}z^{r_\ell-1}+z^{r_\ell}.\]
Set 
\[L_{\cX,\bfr}:=L_\cX\times L_\bfr\qquad\text{ and }\qquad L_{\cA,\bfr}:=L_\cA\times L_\bfr.\]
Here we extend the Poisson structures on $L_\cX$ and $L_\cA$ to $L_{\cX,\bfr}$ and $L_{\cA,\bfr}$ so that the coordinate functions $a_{\ell,j}$ on $L_\bfr$ are Casimirs. 
...
\begin{equation}
  ...
  h_{\cX,\bfr}^{k,\varepsilon}:=-\frac{\varepsilon}{d_k}\int_0^{y_k^\varepsilon} \frac{\log\big(P_k(u)\big)}{u}du\qquad\text{and}\qquad h_{\cA,\bfr}^{k,\varepsilon}:=-\frac{\varepsilon}{d_k}\int_0^{\hat y_k^\varepsilon} \frac{\log\big(P_k(u)\big)}{u}du.
\end{equation}
Observe that when $r_k=1$, i.e.\ $P_k(z)=1+z$, we have $h_{\cX,\bfr}^{k,\varepsilon}:=\frac{\varepsilon}{d_k}\Li_2(-y_k^\varepsilon)$ and $h_{\cA,\bfr}^{k,\varepsilon}:=\frac{\varepsilon}{d_k}\Li_2(-\hat y_k^\varepsilon)$.
...
\begin{lemma}
  ...
  \begin{align}
    \label{eq:X dynamics}
    \dot y_\ell&:=\{h_{\cX,\bfr}^{k,\varepsilon},y_\ell\}_\cX=-B_{k\ell}\log\big(P_k(y_k^\varepsilon)\big)y_\ell\quad\text{for}\quad 1\le\ell\le n;\\
    \label{eq:A dynamics}
    \dot x_j&:=\{h_{\cA,\bfr}^{k,\varepsilon},x_j\}_\cA=-\delta_{jk}\log\big(P_k(\hat y_k^\varepsilon)\big)x_k\quad\text{for}\quad 1\le j\le m.
  \end{align}
...
\begin{proof}
  ...
  \[\{h_{\cA,\bfr}^{k,\varepsilon},x_j\}_\cA=-\frac{\log\big(P_k(\hat y_k^\varepsilon)\big)}{d_k\hat y_k}\{\hat y_k,x_j\}=-\delta_{jk}\log\big(P_k(\hat y_k^\varepsilon)\big)x_j.\]
\end{proof}
...
\begin{corollary}
  ...
  \[(\varphi_{\cX,\bfr}^t)^*(y_\ell)=\big(P_k(y_k^\varepsilon)\big)^{-tB_{k\ell}}y_\ell,\qquad (\varphi_{\cX,\bfr}^t)^*(a_{\ell,i})=a_{\ell,i},\]
  ...
  \[(\varphi_{\cA,\bfr}^t)^*(x_j)=\big(P_k(\hat y_k^\varepsilon)\big)^{-t\delta_{jk}}x_j,\qquad (\varphi_{\cA,\bfr}^t)^*(a_{\ell,i})=a_{\ell,i}\]
...
\begin{lemma}
  ...
  \begin{align}
    \label{eq:X mutation}
    (\mu_{\cX,\bfr}^{k,\varepsilon})^*(y'_\ell)&=\begin{cases} y_k^{-1} & \text{if $\ell=k$;}\\ y_\ell y_k^{[\varepsilon r_kB_{k\ell}]_+}P_k(y_k^\varepsilon)^{-B_{k\ell}} & \text{if $\ell\ne k$;}\end{cases}\\
    \label{eq:A mutation}
    (\mu_{\cA,\bfr}^{k,\varepsilon})^*(x'_j)&=\begin{cases} x_k^{-1}\left(\prod\limits_{i=1}^m x_i^{[-\varepsilon B_{ik}r_k]_+}\right)P_k(\hat y_k^\varepsilon) & \text{if $j=k$;}\\ x_j & \text{if $j\ne k$.}\end{cases}
  \end{align}
...
\begin{remark}
  While we have assumed that $L_\cX=\RR_+^n$, $L_\cA=\RR_+^m$, and $P_k(z)\in\RR_+[z]$ to motivate the Hamiltonian natures of $\mu_{\cX,\bfr}^{k,\varepsilon}$ and~$\mu_{\cA,\bfr}^{k,\varepsilon}$, the cluster mutations \eqref{eq:X mutation} and \eqref{eq:A mutation} are also well-defined when $L_\cX=(\CC^\times)^n$, $L_\cA=(\CC^\times)^m$, and $P_k(z)\in\CC[z]$ is a monic polynomial with $P_k(0)=1$.
  Similarly in the real case, the requirement that the coefficients of $P_k(z)$ be positive real numbers was unnecessary and we allow any monic polynomials $P_k(z)\in\RR[z]$ with $P_k(0)=1$ from now on.
...
\begin{definition}
  ...
  Then, given $\tilde B^t$ mutation equivalent to $\tilde B^{t_0}$, the lower $n\times n$ submatrix $C^t$ of $\tilde B^t$ transforms as follows when $t$ and $t'$ are joined by an edge labeled $k$:
  \begin{equation}
    \label{eq:c-matrix mutation}
    C_{ij}^{t'}=
    \begin{cases}
      -C_{ij}^t & \text{if $j=k$;}\\
      C_{ij}^t+C_{ik}^t[r_kB_{kj}^t]_++[-C_{ik}^t]_+r_kB_{kj}^t & \text{if $j\ne k$.}
    \end{cases}
  \end{equation}
  The $C$-matrices have the following \emph{sign-coherence} property: 
  \begin{itemize}
    \item each column of $C^t$, known as a \emph{$\bfc$-vector}, has either all non-negative entries or all non-positive entries \cite{FZ07,NZ12,GHKK14,NR16}.
  \end{itemize}
...
Given a pair of vertices $t,t'\in\TT_n$, the composition of mutations from $L_{-,\bfr;t}$ to $L_{-,\bfr;t'}$ along the unique path from $t$ to $t'$ in $\TT_n$ (always taken with respect to the tropical sign from Definition~\ref{def:tropical signs}) gives rise to birational maps
\[\mu_{\cX,\bfr}^{t',t}:L_{\cX,\bfr;t}\to L_{\cX,\bfr;t'}\qquad\text{and}\qquad\mu_{\cA,\bfr}^{t',t}:L_{\cA,\bfr;t}\to L_{\cA,\bfr;t'}.\]

When $\tilde B^{t_0}=\tilde B_{prin}$ has principal coefficients, the cluster coordinates of $\cA_\bfr$ admit a lot of structure which turns out to control the expressions for cluster coordinates with arbitrary coefficients.
In this case, all cluster coordinates $\big(\mu_{\cA,\bfr}^{t,t_0}\big)^*(x_{j;t})$ on $L_{\cA,\bfr;t_0}$ are homogeneous with respect to the $\ZZ^n$-grading
\[\deg(x_{i;t_0})=\bfe_i\qquad\text{and}\qquad\deg(x_{n+i;t_0})=-\bfb_i,\]
for $1\le i\le n$, where $\bfe_i$ is the $i$-th standard basis vector of $\ZZ^n$ and $\bfb_i\in\ZZ^n$ is the $i$-th column of $B$.
Note that each $\hat y_k:=x_{n+k;t_0}\prod_{i=1}^n x_{i;t_0}^{B_{ik}^{t_0}}$ has degree zero under this grading.
The \emph{$\bfg$-vector} $\bfg_{j;t}=\sumlimits_{i=1}^n G_{ij}^t\bfe_i$ of the cluster coordinate $x_{j;t}$ is its degree when pulled back to $L_{\cA,\bfr;t_0}$.
These satisfy the following recursion for vertices $t,t'\in\TT_n$ which are joined by an edge labeled $k$:
\begin{equation}
  \label{eq:g-matrix mutation}
  G_{ij}^{t'}=
  \begin{cases}
    -G_{ik}^t+\sum\limits_{\ell=1}^n G_{i\ell}^t[-B_{\ell k}^t r_k]_+-\sum\limits_{\ell=1}^n B_{i\ell}^t[-C_{\ell k}^t r_k]_+ & \text{if $j=k$;}\\
    G_{ij}^t & \text{if $j\ne k$.}
  \end{cases}
\end{equation}
Then there exists a unique polynomial $F_{j,\bfr;t}\in\CC[u_1,\ldots,u_n]$, called the \emph{$F$-polynomial}, such that the initial cluster Laurent expansion of $x_{j;t}$ is given by
\[\big(\mu_{\cX,\bfr}^{t,t_0}\big)^*(x_{j;t})=\prod_{i=1}^n x_i^{G_{ij}^t} F_{j,\bfr;t}(\hat y_1,\ldots,\hat y_n).\]
...
\begin{equation}
  ...
  F_{j,\bfr;t'}=
  \begin{cases}
    F_{k,\bfr;t}^{-1}\left(\prod_{i=1}^n u_i^{[-C_{ik}^t]_+} F_{i,\bfr;t}^{[-B_{ik}^t]_+}\right)^{r_k} P_k\left(\prod_{i=1}^n u_i^{C_{ik}^t} F_{i,\bfr;t}^{B_{ik}^t}\right) & \text{if $j=k$;}\\
    F_{j,\bfr;t} & \text{if $j\ne k$.}
  \end{cases}
\end{equation}
...
Let $\tilde B^{t_0}$ be an $m\times n$ exchange matrix.
Define a \emph{tropical addition} on monomials in $\CC[x_{j;t_0}:n+1\le j\le m]$ by
\[\prod_{j=n+1}^m x_{j;t_0}^{a_j}\oplus \prod_{j=n+1}^m x_{j;t_0}^{b_j}=\prod_{j=n+1}^m x_{j;t_0}^{\max\{a_j,b_j\}}.\]
Then we have the following \emph{separation of additions} formulas:
\begin{align}
  \label{eq:separation of additions 1}
  \big(\mu_{\cA,\bfr}^{t,t_0}\big)^*(x_{j;t})=\prod_{i=1}^n x_i^{G_{ij}^t} \frac{F_{j,\bfr;t}(\hat y_1,\ldots,\hat y_n)}{F_{j,\bfr;t}^{\oplus}(\hat y_1,\ldots,\hat y_n)};\\
...
\begin{align}
  ...
  H_{\cX,\bfr}^{k,\varepsilon}&:=\frac{\varepsilon}{d_k}\int_{\alpha^*(y_k^\varepsilon)}^{\beta^*(y_k^\varepsilon)} \frac{\log\big(P_k(u)\big)}{u}du;\\
  \label{eq:A hamiltonian}
  H_{\cA,\bfr}^{k,\varepsilon}&:=\frac{\varepsilon}{d_k}\int_{\alpha^*(\hat y_k^\varepsilon)}^{\beta^*(\hat y_k^\varepsilon)} \frac{\log\big(P_k(u)\big)}{u}du.
\end{align}
...
\begin{lemma}
  ...
  \begin{align}
    \dot q_\ell&=\{H_{\cX,\bfr}^{k,\varepsilon},q_\ell\}_{\cG_\cX}=-\frac{\delta_{k\ell}}{d_ky_k}\log\left(\frac{P_k\left(y_k^\varepsilon e^{\varepsilon\sum_{\ell'=1}^n d_{\ell'} B_{\ell' k}q_{\ell'} y_{\ell'}}\right)}{P_k(y_k^\varepsilon)}\right)+B_{k\ell}\log\big(P_k(y_k^\varepsilon)\big)q_\ell;\\
    \dot v_\ell&=\{H_{\cX,\bfr}^{k,\varepsilon},v_\ell\}_{\cB_\cX}=-\frac{\delta_{k\ell}}{d_ky_k}\log\left(\frac{P_k\left(y_k^\varepsilon \prod_{\ell'=1}^n (1+v_{\ell'}y_{\ell'})^{\varepsilon d_{\ell'} B_{\ell' k}}\right)}{P_k(y_k^\varepsilon)}\right)(v_ky_k+1)+B_{k\ell}\log\big(P_k(y_k^\varepsilon)\big)v_\ell;\\
    \dot t_\ell&=\{H_{\cX,\bfr}^{k,\varepsilon},t_\ell\}_{\cD_\cX}=-\frac{\delta_{k\ell}}{d_k}\log\left(\frac{P_k\left(y_k^\varepsilon \prod_{\ell'=1}^nt_{\ell'}^{\varepsilon d_{\ell'} B_{\ell' k}}\right)}{P_k(y_k^\varepsilon)}\right)t_k;
  \end{align}
  ...
  \begin{align}
    \label{eq:p dot}
    \dot p_j&=\{H_{\cA,\bfr}^{k,\varepsilon},p_j\}_{\cG_\cA}=-\frac{B_{jk}}{d_kx_j}\log\left(\frac{P_k\left(\hat y_k^\varepsilon e^{-\varepsilon d_kp_kx_k}\right)}{P_k(\hat y_k^\varepsilon)}\right)+\delta_{jk}\log\big(P_k(\hat y_k^\varepsilon)\big)p_k;\\
    \label{eq:u dot}
    \dot u_j&=\{H_{\cA,\bfr}^{k,\varepsilon},u_j\}_{\cB_\cA}=-\frac{B_{jk}}{d_kx_j}\log\left(\frac{P_k\left(\hat y_k^\varepsilon (1+u_kx_k)^{-\varepsilon d_k}\right)}{P_k(\hat y_k^\varepsilon)}\right)(u_jx_j+1)+\delta_{jk}\log\big(P_k(\hat y_k^\varepsilon)\big)u_k\\
    \label{eq:s dot}
    \dot s_j&=\{H_{\cA,\bfr}^{k,\varepsilon},s_j\}_{\cD_\cA}=-\frac{B_{jk}}{d_k}\log\left(\frac{P_k\left(\hat y_k^\varepsilon s_k^{-\varepsilon d_k}\right)}{P_k(\hat y_k^\varepsilon)}\right)s_j.
  \end{align}
  The coordinates $b_{\ell,j}$ on $T^*L_\bfr$ evolve according to
  \begin{equation}
    \label{eq:b dot}
    \dot b_{\ell,j}=\frac{\varepsilon}{d_k}\int_*^* \frac{u^{j-1}}{P_k(u)}du,
  \end{equation}
...
\begin{corollary}
  ...
  \begin{align*}
    (\varphi_{\cX,\bfr}^t)^*(q_\ell y_\ell)&=q_\ell y_\ell-t\frac{\delta_{k\ell}}{d_k}\log\left(\frac{P_k\left(y_k^\varepsilon e^{\varepsilon\sum_{\ell'=1}^n d_{\ell'} B_{\ell' k}q_{\ell'} y_{\ell'}}\right)}{P_k(y_k^\varepsilon)}\right),\\
    (\varphi_{\cX,\bfr}^t)^*(v_\ell y_\ell)&=\left(\frac{P_k\left(y_k^\varepsilon \prod_{\ell'=1}^n (v_{\ell'} y_{\ell'} + 1)^{\varepsilon d_{\ell'} B_{\ell' k}}\right)}{P_k(y_k^\varepsilon)}\right)^{-t\frac{\delta_{k\ell}}{d_k}}(v_\ell y_\ell+1)-1,\\
    (\varphi_{\cX,\bfr}^t)^*(t_\ell)&=\left(\frac{P_k\left(y_k^\varepsilon \prod_{\ell'=1}^n t_{\ell'}^{\varepsilon d_{\ell'} B_{\ell' k}}\right)}{P_k(y_k^\varepsilon)}\right)^{-t\delta_{k\ell}/d_k}t_\ell;
  \end{align*}
  and the Hamiltonian flow $\varphi_{\cA,\bfr}^t: L_{\cA,\bfr} \to L_{\cA,\bfr}$ in Corollary~\ref{cor:time-one flows} is lifted to the multiplicative Hamiltonian flows $\varphi_{\cA,\bfr}^t: \cG_{\cA,\bfr} \to \cG_{\cA,\bfr}$, $\varphi_{\cA,\bfr}^t: \cB_{\cA,\bfr} \to \cB_{\cA,\bfr}$, $\varphi_{\cA,\bfr}^t: \cD_{\cA,\bfr} \to \cD_{\cA,\bfr}$, given on coordinates by
  \begin{align*}
    (\varphi_{\cA,\bfr}^t)^*(p_jx_j)&=p_jx_j-t\frac{B_{jk}}{d_k}\log\left(\frac{P_k\left(\hat y_k^\varepsilon e^{-\varepsilon d_kp_kx_k}\right)}{P_k(\hat y_k^\varepsilon)}\right), \\
    (\varphi_{\cA,\bfr}^t)^*(u_jx_j)&=\left(\frac{P_k\left(\hat y_k^\varepsilon (u_kx_k+1)^{-\varepsilon d_k}\right)}{P_k(\hat y_k^\varepsilon)}\right)^{-t\frac{B_{jk}}{d_k}}(u_jx_j+1)-1, \\
    (\varphi_{\cA,\bfr}^t)^*(s_j)&=\left(\frac{P_k\left(\hat y_k^\varepsilon s_k^{-\varepsilon d_k}\right)}{P_k(\hat y_k^\varepsilon)}\right)^{-tB_{jk}/d_k}s_j.
  \end{align*}
  Each of the Hamiltonian flows above transform the coordinates $b_{\ell,j}$ on $T^*L_\bfr$ according to
  \begin{equation*}
    (\varphi_{-,\bfr}^t)^*(b_{\ell,j})=b_{\ell,j}+\frac{\varepsilon}{d_k}\int_*^* \frac{u^{j-1}}{P_k(u)}du,
  \end{equation*}
  where the bounds of integration are respectively those from equations \eqref{eq:X hamiltonian} or \eqref{eq:A hamiltonian}.
\end{corollary}
\begin{proof}
...
\begin{lemma}
  ...
  \begin{align*}
    \tau_\cX^{k,\varepsilon}&:\cG_\cX\to\cG'_\cX,& \tau_\cX^{k,\varepsilon}&:\cB_\cX\to\cB'_\cX,& \bar\tau_\cX^{k,\varepsilon}&:\cD_\cX\to\cD'_\cX\\
    \tau_\cA^{k,\varepsilon}&:\cG_\cA\to\cG'_\cA,& \tau_\cA^{k,\varepsilon}&:\cB_\cA\to\cB'_\cA,& \tau_\cA^{k,\varepsilon}&:\cD_\cA\to\cD'_\cA
  \end{align*}
  lifting respectively the Poisson morphisms $\tau_\cX^{k,\varepsilon}:L_\cX\to L'_\cX$ and $\tau_\cA^{k,\varepsilon}:L_\cA\to L'_\cA$ of Lemma~\ref{le:tropical cluster transformations}.
  These are given on coordinates by
  \begin{align}
    \label{eq:tropical GX transformation}
    (\tau_\cX^{k,\varepsilon})^*(q'_\ell)
    &=\begin{cases} 
    ...
    (\tau_\cX^{k,\varepsilon})^*(v'_\ell)
    &=\begin{cases} 
      y_k\Big[(v_k y_k + 1)^{-1}\prod\limits_{\ell'=1}^n (v_{\ell'} y_{\ell'} + 1)^{[\varepsilon B_{k\ell'}]_+} -1\Big] & \text{if $\ell=k$;}\\
      v_\ell y_k^{-[\varepsilon B_{k\ell}]_+} & \text{if $\ell\ne k$;}
    \end{cases}\\
    \label{eq:tropical DX transformation}
    (\tau_\cX^{k,\varepsilon})^*(t'_\ell)
    &=\begin{cases} 
      t_k^{-1}\prod\limits_{\ell'=1}^n t_{\ell'}^{[\varepsilon B_{k\ell'}]_+} & \text{if $\ell=k$;}\\
      t_\ell & \text{if $\ell\ne k$;}
    \end{cases}\\
    \label{eq:tropical GA transformation}
    (\tau_\cA^{k,\varepsilon})^*(p'_j)&=
    \begin{cases} 
    ...
    (\tau_\cA^{k,\varepsilon})^*(u'_j)
    &=\begin{cases} 
      -(u_k x_k + 1)^{-1}u_k x_k^2 \prod\limits_{i=1}^m x_i^{-[-\varepsilon B_{ik}]_+}  & \text{if $j=k$;}\\ 
      x_j^{-1}\big[(u_j x_j + 1) (u_k x_k + 1)^{[-\varepsilon B_{jk}]_+}-1\big] & \text{if $j\ne k$.}
    \end{cases}\\
    \label{eq:tropical DA transformation}
    (\tau_\cA^{k,\varepsilon})^*(s'_j)
    &=\begin{cases} 
      s_k^{-1} & \text{if $j=k$;}\\ 
...
\begin{proposition}
  ...
  \begin{align}
    ...
    &=\begin{cases} 
      -q_k y_k + \sum\limits_{\ell'=1}^n [\varepsilon B_{k\ell'}]_+ q_{\ell'} y_{\ell'} + \frac{1}{d_k}\log\left(\frac{P_k\left(y_k^\varepsilon e^{\varepsilon\sum_{\ell'=1}^n d_{\ell'} B_{\ell' k}q_{\ell'} y_{\ell'}}\right)}{P_k(y_k^\varepsilon)}\right) & \text{if $\ell=k$;}\\ 
    ...
    \begin{cases} 
      -p_k x_k & \text{if $j=k$;}\\ 
      p_j x_j + [-\varepsilon B_{jk}]_+ p_k x_k - \frac{B_{jk}}{d_k}\log\left(\frac{P_k\left(\hat y_k^\varepsilon e^{-\varepsilon d_kp_kx_k}\right)}{P_k(\hat y_k^\varepsilon)}\right) & \text{if $j\ne k$;}
    \end{cases}\\
    \label{eq:DX mutation}
    (\bar\mu_{\cX,\bfr}^{k,\varepsilon})^*(t'_\ell)
    &=\begin{cases} 
      t_k^{-1}\prod\limits_{\ell'=1}^n t_{\ell'}^{[\varepsilon B_{k\ell'}]_+}\left(\frac{P_k\left(y_k^\varepsilon \prod_{\ell'=1}^n t_{\ell'}^{\varepsilon d_{\ell'} B_{\ell' k}}\right)}{P_k(y_k^\varepsilon)}\right)^{1/d_k} & \text{if $\ell=k$;}\\
      t_\ell & \text{if $\ell\ne k$;}
    \end{cases}\\
    ...
    &=\begin{cases} 
      s_k^{-1} & \text{if $j=k$;}\\ 
      s_j s_k^{[-\varepsilon B_{jk}]_+} \left(\frac{P_k\left(\hat y_k^\varepsilon s_k^{-\varepsilon d_k}\right)}{P_k(\hat y_k^\varepsilon)}\right)^{-B_{jk}/d_k} & \text{if $j\ne k$.}
    \end{cases}
  \end{align}
\end{proposition}
...
\begin{equation} 
	\label{eq:GpdCD}
    \xymatrix{
      \cG_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\varphi_{\cA,\bfr}} \ar@/^1pc/[rr]|-{\mu_{\cA,\bfr}} & \cG_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\tau_{\cA,\bfr}} & \cG'_{\cA,\bfr} \ar[d]^{\rho} 
	&& \cD_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\varphi_{\cA,\bfr}} \ar@/^1pc/[rr]|-{\mu_{\cA,\bfr}} & \cD_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\tau_{\cA,\bfr}} & \cD'_{\cA,\bfr} \ar[d]^{\rho} \\
      \cG_{\cX,\bfr} \ar[r]^{\varphi_{\cX,\bfr}} \ar@/_1pc/[rr]|-{\mu_{\cX,\bfr}} & \cG_{\cX,\bfr} \ar[r]^{\tau_{\cX,\bfr}} \ar@{}[ddr]^(.325){}="a"^(.775){}="b" \ar@<1ex> "a";"b"^-{\kappa_\cA} \ar@<-1ex> "a";"b"_-{\kappa_\cX} & \cG'_{\cX,\bfr} \ar@{}[rr]^(.3){}="e"^(.7){}="f" \ar@<5ex> "e";"f"^-{\lambda_\cA} \ar@<3ex> "e";"f"_-{\lambda_\cX}
	&& \cD_{\cX,\bfr} \ar[r]^{\varphi_{\cX,\bfr}} \ar@/_1pc/[rr]|-{\mu_{\cX,\bfr}} & \cD_{\cX,\bfr} \ar[r]^{\tau_{\cX,\bfr}} & \cD'_{\cX,\bfr} \\
	&&&&&&\\
	&&\cB_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\varphi_{\cA,\bfr}} \ar@/^1pc/[rr]|-{\mu_{\cA,\bfr}} & \cB_{\cA,\bfr} \ar[d]^{\rho} \ar[r]_{\tau_{\cA,\bfr}} & \cB'_{\cA,\bfr} \ar[d]^{\rho} \ar@{}[uur]^(.2){}="c"^(.65){}="d" \ar@<1ex> "c";"d"^-{\nu_\cA} \ar@<-1ex> "c";"d"_-{\nu_\cX}  \\
      &&\cB_{\cX,\bfr} \ar[r]^{\varphi_{\cX,\bfr}} \ar@/_1pc/[rr]|-{\mu_{\cX,\bfr}} & \cB_{\cX,\bfr} \ar[r]^{\tau_{\cX,\bfr}} & \cB'_{\cX,\bfr}
    }
\end{equation}
