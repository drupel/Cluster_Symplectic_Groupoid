...
\end{enumerate}	
The two notions are related by the formula: $\{f, g\} = \pi (df \otimes dg)$, for $f, g\in \cO_\cX$. The pair $(M, \pi)$, or equivalently $(M, \{,\})$, is called a Poisson manifold.
\end{definition}
...
%		\varphi_X^t: (\bfp, \bfx) \mapsto \left(e^{-t\sum\limits_{i} \Omega_{1i} x_ip_i} p_1, \ldots, e^{-t\sum\limits_{i} \Omega_{mi} x_ip_i} p_m, e^{t\sum\limits_{i} \Omega_{1i} x_ip_i} x_1, \ldots, e^{t\sum\limits_{i} \Omega_{mi} x_ip_i} x_m\right)
	$$
The flow exists for all time $t \in \RR$ and has no loops. It is straightforward to check that the source-simply-connected symplectic groupoid structures are induced by the Poisson spray \eqref{eq: PoisSp}. Note that the multiplicative 2-form $\omega$ is non-degenerate since
	$$
...
Motivated by the terminology of Fock and Goncharov \cite{???}, we will refer to the pair of Poisson manifolds $L_\cX$ and $L_\cA$ together with the Poisson morhism $\rho$ as a \emph{Poisson ensemble} associated to the $D$-compatible pair $(\tilde B,\Omega)$.
...
Write $X_\cX=X_\cX^{k,\varepsilon}\in\cT_\cX$ and $X_\cA=X_\cA^{k,\varepsilon}\in\cT_\cA$ for the Hamiltonian vector fields associated to $H_\cX^{k,\varepsilon}$ and $H_\cA^{k,\varepsilon}$ respectively, i.e.\ the vector fields associated to the derivations $\{H_\cX^{k,\varepsilon},\cdot\}_\cX$ and $\{H_\cA^{k,\varepsilon},\cdot\}_\cA$.
\begin{lemma}
...
\end{lemma}
\begin{proof}
  By skew-symmetry of the Poisson brackets, there are essentially only two cases to check.
  For $\ell,\ell'\ne k$, we have
  \begin{align*}
    \{(\tau_\cX^{k,\varepsilon})^*(y'_\ell),(\tau_\cX^{k,\varepsilon})^*(y'_{\ell'})\}_\cX
    &=\{y_\ell y_k^{[\varepsilon B_{k\ell}]_+},y_{\ell'} y_k^{[\varepsilon B_{k\ell'}]_+}\}_\cX\\
  ...
  \begin{align*}
    ...
    &=\{x_k^{-1}\left(\prod\limits_{i=1}^m x_i^{[-\varepsilon B_{ik}]_+}\right),x_j\}_\cA\\
    &=(-\Omega_{kj}+\sum\limits_{i=1}^m [-\varepsilon B_{ik}]_+\Omega_{ij})x_k^{-1}\left(\prod\limits_{i=1}^m x_i^{[-\varepsilon B_{ik}]_+}\right)x_j\\
