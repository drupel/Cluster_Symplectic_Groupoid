...
$$
For $(M, \pi)$, we denote its cotangent Lie algebroid by $T^*_\pi M$. As a Poisson groupoid, $(\cG, \omega) \rra (M, \pi)$ integrates the Lie bialgebroid $(T^*_\pi M, TM)$ \cite{MR1262213}.
...
\begin{remark}
	If $L = \RR^m$, then $\cB$ is already source-simply-connected and $\kappa: \cG \to \cB$ is a groupoid isomorphism, but if $L = \CC^m$, then $\cB$ is not source-simply-connected and $\kappa: \cG \to \cB$ is a covering map for each source fiber.
\end{remark}
