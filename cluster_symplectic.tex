...
\begin{definition} \label{def:symp realization}
For a Poisson manifold $(M, \pi)$, a symplectic realization is a symplectic manifold $(S, \omega)$ together with a surjecive Poisson map $(S, \omega) \to (M, \pi)$.
\end{definition}
...
\begin{definition}
  A \emph{Poisson groupoid} is a Lie groupoid $\cG \rra M$ with a multiplicative Poisson structure $\sigma \in \cT^2_\cG$. That is, the graph of the multiplication map $\Gamma_m$ is coisotropic with repsect to the Poisson structure $\sigma \oplus \sigma \oplus -\sigma$.
\end{definition}

A Poisson groupoid map from $(\cG_1, \sigma_1) \rra M_1$ to $(\cG_2, \sigma_2) \rra M_2$ is a Lie groupoid map $\varphi: \cG_1 \to \cG_2$ which is also Poisson.

For a Poisson groupoid $(\cG, \sigma) \rra M$, there is a natural Poisson structure $\pi$ on $M$ such that $\alpha: (\cG, \sigma) \to (M, \pi)$ and $\beta: (\cG, \sigma) \to (M, -\pi)$ are Poisson maps. In particular, for a Poisson manifold $(M, \pi)$, one could ask if there exists a symplectic groupoid $(\cG, \omega) \rra M$ such that $\alpha: (\cG, \omega) \to (M, \pi)$ is Poisson. If the answer is yes, then we say the Poisson manifolds $(M, \pi)$ is \emph{integrable}. The Lie algeboid of a symplectic groupoid $\cG \rra M$ is the cotangent Lie algebroid $T^*M$ \cite{MR866024} with the anchor map
$$
...
$$
We denote this cotangent Lie algebroid by $T^*_\pi M$.

As a Poisson groupoid, $(\cG, \omega) \rra (M, \pi)$ integrates the Lie bialgebroid $(T^*_\pi M, TM)$, where $TM$ carries the standard Lie algebroid bracket of vector fields \cite{MR1262213}.
...
For the standard symplectic structure $\omega_0 = \sum_{i} d\chi_i \wedge d\xi_i$ on $\cU$, since
	...
	\]
is supported on the $\alpha$-fiber, it follows that $\beta_*(-\omega_0^{-1}) = \beta_*(-\omega_\cU^{-1}) = \pi$. The symplectic realization $\beta: (\cU, -\omega_0) \to (L, \pi)$ (see Definition~\ref{def:symp realization}) plays an important role in \cite{MR3691969}.
\end{remark}
...
\]
For a $m\times m$ skew-symmetric matrix $\Omega$, we define a Lie algebroid structure on $A_\cD$ with the anchor map
\[
...
Let $\cG \rra M$ be a Lie groupoid and let $\cH \rra L$ be a Lie subgroupoid over a hypersurface $L$. We denote the blowup of $\cG$ along $\cH$ by $\mathrm{Bl}(\cG, \cH)$ with the blow-down map $p: \mathrm{Bl}(\cG, \cH) \to \cG$ and write $[\cG\!:\!\cH] = \mathrm{Bl}(\cG, \cH) \setminus (S_L \cup T_L)$ where $S_L$ is the proper transform of $\alpha^{-1}(L)$ and $T_L$ is the proper transform of $\beta^{-1}(L)$.

\begin{theorem} \cite{MR3214314}
	For a Lie groupoid $\cG \rra M$ and a Lie subgroupoid $\cH \rra L$ for some hypersurface $L$, there is a unique Lie groupoid structure $[\cG\!:\!\cH] \rra M$ such that the blow-down map $p: [\cG\!:\!\cH] \to \cG$ is a groupoid morphism. Moreover, blowup of Lie groupoids corresponds to elementary modification of Lie algebroids. That is,
	\[
